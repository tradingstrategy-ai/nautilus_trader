...
from nautilus_trader.adapters.aster.common.schemas.market import AsterAggregatedTradeMsg
from nautilus_trader.adapters.aster.common.schemas.market import AsterCandlestickMsg
from nautilus_trader.adapters.aster.common.schemas.market import AsterDataMsgWrapper
from nautilus_trader.adapters.aster.common.schemas.market import AsterOrderBookMsg
from nautilus_trader.adapters.aster.common.schemas.market import AsterQuoteMsg
from nautilus_trader.adapters.aster.common.schemas.market import AsterTickerMsg
...
from nautilus_trader.adapters.aster.websocket.client import AsterWebSocketClient
...
class AsterCommonDataClient(LiveMarketDataClient):
    """
    Provides a data client of common methods for the Aster exchange.
    ...
    def __init__(
        ...
        # Configuration
        self._aster_account_type = account_type
        ...
        # WebSocket API
        self._ws_client = AsterWebSocketClient(
            clock=clock,
        ...
        self._decoder_data_msg_wrapper = msgspec.json.Decoder(AsterDataMsgWrapper)
        self._decoder_order_book_msg = msgspec.json.Decoder(AsterOrderBookMsg)
        self._decoder_quote_msg = msgspec.json.Decoder(AsterQuoteMsg)
        self._decoder_ticker_msg = msgspec.json.Decoder(AsterTickerMsg)
        self._decoder_candlestick_msg = msgspec.json.Decoder(AsterCandlestickMsg)
        self._decoder_agg_trade_msg = msgspec.json.Decoder(AsterAggregatedTradeMsg)

        # Retry logic (hardcoded for now)
        self._max_retries: int = 3
        self._retry_delay: float = 1.0
        self._retry_errors: set[AsterErrorCode] = {
    ...
    def _should_retry(self, error_code: AsterErrorCode, retries: int) -> bool:
        return not (
    ...
    async def _subscribe(self, command: SubscribeData) -> None:
        ...
        if command.data_type.type == AsterTicker:
            await self._ws_client.subscribe_ticker(instrument_id.symbol.value)
        elif command.data_type.type == AsterFuturesMarkPriceUpdate:
            if not self._aster_account_type.is_futures:
                self._log.error(
                    "Cannot subscribe to `AsterFuturesMarkPriceUpdate` "
                    f"for {self._aster_account_type.value} account types",
                )
                return
    ...
    async def _unsubscribe(self, command: UnsubscribeData) -> None:
        ...
        if (
            ...
            self._log.error(
                "Cannot unsubscribe to `AsterFuturesMarkPriceUpdate` no instrument ID in `data_type` metadata",
            )
            return
        ...
        if command.data_type.type == AsterTicker:
            ...
            if not self._aster_account_type.is_futures:
                self._log.error(
                    "Cannot unsubscribe from `AsterFuturesMarkPriceUpdate` "
                    f"for {self._aster_account_type.value} account types",
                )
                return
        else:
    ...
    async def _subscribe_order_book(self, command: SubscribeOrderBook) -> None:
        ...
        if command.book_type == BookType.L3_MBO:
            self._log.error(
                ...
                "L3_MBO data is not published by Aster. "
        ...
        # Aster maximum (1000). Note that a depth of ``0`` means *full book*
        # in NautilusTrader semantics, which we translate to 1000; the maximum
        # value accepted by the Aster partial book snapshot endpoint.
    ...
    async def _subscribe_bars(self, command: SubscribeBars) -> None:
        ...
        if not command.bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot subscribe to {command.bar_type}: only time bars are aggregated by Aster",
            )
            return
        ...
        if self._aster_account_type.is_futures and resolution == "s":
            self._log.error(
                f"Cannot subscribe to {command.bar_type}. "
                "Second interval bars are not aggregated by Aster Futures",
            )
        try:
            interval = AsterKlineInterval(f"{command.bar_type.spec.step}{resolution}")
        except ValueError:
            self._log.error(
                f"Bar interval {command.bar_type.spec.step}{resolution} not supported by Aster",
            )
            return
    ...
    async def _unsubscribe_order_book_deltas(self, command: UnsubscribeOrderBook) -> None:
        pass  # TODO: Unsubscribe from Aster if no other subscriptions
    ...
    async def _unsubscribe_bars(self, command: UnsubscribeBars) -> None:
        if not command.bar_type.spec.is_time_aggregated():
            self._log.error(
                f"Cannot unsubscribe from {command.bar_type}: only time bars are aggregated by Aster",
            )
            return
    ...
    async def _request_quote_ticks(self, request: RequestQuoteTicks) -> None:
        self._log.error(
            "Cannot request historical quotes: not published by Aster",
        )
    ...
    async def _request_bars(self, request: RequestBars) -> None:
        if request.bar_type.spec.price_type != PriceType.LAST:
            self._log.error(
                ...
                f"only historical bars for LAST price type available from Aster",
            )
            return
        ...
        if (
            ...
            if not request.bar_type.spec.is_time_aggregated():
                self._log.error(
                    f"Cannot request {request.bar_type} bars: only time bars are aggregated by Aster",
                )
                return
            ...
            if not self._aster_account_type.is_spot_or_margin and resolution == "s":
                self._log.error(
                    f"Cannot request {request.bar_type} bars: "
                    "second interval bars are not aggregated by Aster Futures",
                )
            try:
                interval = AsterKlineInterval(f"{request.bar_type.spec.step}{resolution}")
            except ValueError:
                self._log.error(
                    f"Cannot create Aster Kline interval. {request.bar_type.spec.step}{resolution} "
                    "not supported",
                )
                return
        ...
        # Aster may return the current forming bar which should be excluded from historical data
    ...
    async def _aggregate_internal_from_minute_bars(
        ...
        self._log.info("Requesting 1-MINUTE Aster bars to infer INTERNAL bars...", LogColor.BLUE)

        aster_bars = await self._http_market.request_aster_bars(
            bar_type=BarType(
                bar_type.instrument_id,
                BarSpecification(1, BarAggregation.MINUTE, PriceType.LAST),
                AggregationSource.EXTERNAL,
            ),
            interval=AsterKlineInterval.MINUTE_1,
            start_time=start_time_ms,
            end_time=end_time_ms,
            limit=limit,
        )
        ...
        if bar_type.spec.aggregation == BarAggregation.TICK:
            ...
            raise NotImplementedError(f"Inferring bars from Aster klines failed: {msg}")

        for aster_bar in aster_bars:
            if aster_bar.count == 0:
                continue
        ...
        self._log.info(
            f"Inferred {len(bars)} {bar_type} bars aggregated from {len(aster_bars)} 1-MINUTE Aster bars",
            LogColor.BLUE,
        )
    ...
    def _aggregate_bar_to_trade_ticks(
        ...
        volume = aster_bar.volume.as_decimal()
        size_part: Decimal = (volume / (4 * aster_bar.count)).quantize(
            quantize_value,
            rounding=decimal.ROUND_DOWN,
        )
        remainder: Decimal = volume - (size_part * 4 * aster_bar.count)

        size = Quantity(size_part, instrument.size_precision)

        for i in range(aster_bar.count):
            open = TradeTick(
                instrument_id=instrument.id,
                price=aster_bar.open,
                size=size,
                aggressor_side=AggressorSide.NO_AGGRESSOR,
                trade_id=TradeId("NULL"),  # N/A
                ts_event=aster_bar.ts_event,
                ts_init=aster_bar.ts_event,
            )

            high = TradeTick(
                instrument_id=instrument.id,
                price=aster_bar.high,
                size=size,
                aggressor_side=AggressorSide.NO_AGGRESSOR,
                trade_id=TradeId("NULL"),  # N/A
                ts_event=aster_bar.ts_event,
    ...
    async def _aggregate_internal_from_agg_trade_ticks(
        ...
        if bar_type.spec.aggregation == BarAggregation.TICK:
            ...
            raise NotImplementedError(
                f"Inferring bars from Aster aggregated trades failed: {msg}",
            )
    ...
    def _get_cached_instrument_id(self, symbol: str) -> InstrumentId:
        # Parse instrument ID
        aster_symbol = AsterSymbol(symbol)
        nautilus_symbol: str = aster_symbol.parse_as_nautilus(
    ...
    def _handle_ticker(self, raw: bytes) -> None:
        ...
        instrument_id: InstrumentId = self._get_cached_instrument_id(msg.data.s)
        ticker: AsterTicker = msg.data.parse_to_aster_ticker(
            instrument_id=instrument_id,
            ts_init=self._clock.timestamp_ns(),
        )
        data_type = DataType(
    ...
    def _handle_kline(self, raw: bytes) -> None:
        ...
        bar: AsterBar = msg.data.k.parse_to_aster_bar(
            instrument_id=instrument_id,
