...
def enc_hook(obj: Any) -> Any:
    if isinstance(obj, (AsterSymbol, AsterSymbols)):
        return str(obj)  # serialize AsterSymbol as string.
    else:
        raise TypeError(f"Objects of type {type(obj)} are not supported")


class AsterHttpEndpoint:
    """
    Base functionality of endpoints connecting to the Aster REST API.
    ...
    def __init__(
        ...
        methods_desc: dict[HttpMethod, AsterSecurityType],
        url_path: str,
    ):
        self.client = client
        self.methods_desc = methods_desc
        self.url_path = url_path
        self._ratelimiter_key = f"aster:{url_path}"

        self.decoder = msgspec.json.Decoder()
        self.encoder = msgspec.json.Encoder(enc_hook=enc_hook)

        self._method_request = {
            AsterSecurityType.NONE: self.client.send_request,
            AsterSecurityType.USER_STREAM: self.client.send_request,
            AsterSecurityType.MARKET_DATA: self.client.send_request,
            AsterSecurityType.TRADE: self.client.sign_request,
            AsterSecurityType.MARGIN: self.client.sign_request,
            AsterSecurityType.USER_DATA: self.client.sign_request,
        }
    ...
    async def _method(
        ...
        default_keys.append("aster:global")
