...
class AsterUserTrade(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Spot/Margin `GET /api/v3/myTrades` HTTP response from
    Aster USD-M Futures `GET /fapi/v1/userTrades` HTTP response from Aster COIN-M
    ...
    # Parameters not present in 'fills' list (see FULL response of AsterOrder)
...
class AsterOrder(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Spot/Margin `GET /api/v3/order` HTTP response from
    Aster USD-M Futures `GET /fapi/v1/order` HTTP response from Aster COIN-M Futures
    `GET /dapi/v1/order`.
    """
    ...
    # Parameters in SPOT/MARGIN only:
    orderListId: int | None = None  # Unless OCO, the value will always be -1
    cummulativeQuoteQty: str | None = None  # Aster uses double 'm' (their typo)
    ...
    fills: list[AsterUserTrade] | None = None  # FULL response only

    # Parameters in FUTURES only:
    avgPrice: str | None = None
    origType: AsterOrderType | None = None
    ...
    def _parse_time_in_force_and_expire(
        ...
        time_in_force = (
            enum_parser.parse_aster_time_in_force(self.timeInForce) if self.timeInForce else None
        )
    ...
    def parse_to_order_status_report(
        ...
        if self.price is None:
            raise RuntimeError(
                "Cannot generate order status report from Aster ACK response.",
            )
        ...
        post_only = (
            self.type == AsterOrderType.LIMIT_MAKER or self.timeInForce == AsterTimeInForce.GTX
        )
        ...
        order_status = enum_parser.parse_aster_order_status(self.status)
        if treat_expired_as_canceled and order_status == OrderStatus.EXPIRED:
            order_status = OrderStatus.CANCELED

        order_type = enum_parser.parse_aster_order_type(self.type)
        ...
        return OrderStatusReport(
            ...
            venue_order_id=VenueOrderId(str(self.orderId)),
            order_side=enum_parser.parse_aster_order_side(self.side),
...
class AsterStatusCode(msgspec.Struct, frozen=True):
    """
