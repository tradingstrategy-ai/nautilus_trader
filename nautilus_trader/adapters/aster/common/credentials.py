...
def get_api_key(account_type: AsterAccountType, environment: AsterEnvironment) -> str:
    """
    Get Aster API key from environment variables.
    ...
    if environment == AsterEnvironment.TESTNET:
        if account_type.is_spot_or_margin:
            return _get_credential(
                standard_key="ASTER_TESTNET_API_KEY",
                deprecated_key="ASTER_TESTNET_ED25519_API_KEY",
            )
        else:
            return _get_credential_soft(
                standard_key="ASTER_FUTURES_TESTNET_API_KEY",
                deprecated_key="ASTER_FUTURES_TESTNET_ED25519_API_KEY",
            )

    if environment == AsterEnvironment.DEMO:
        return get_env_key("ASTER_DEMO_API_KEY")

    _resolve = _get_credential if account_type.is_spot_or_margin else _get_credential_soft
    return _resolve(
        standard_key="ASTER_API_KEY",
        deprecated_key="ASTER_ED25519_API_KEY",  # gitleaks:allow
    )


def get_api_secret(account_type: AsterAccountType, environment: AsterEnvironment) -> str:
    """
    Get Aster API secret from environment variables.
    ...
    if environment == AsterEnvironment.TESTNET:
        if account_type.is_spot_or_margin:
            return _get_credential(
                standard_key="ASTER_TESTNET_API_SECRET",
                deprecated_key="ASTER_TESTNET_ED25519_API_SECRET",
            )
        else:
            return _get_credential_soft(
                standard_key="ASTER_FUTURES_TESTNET_API_SECRET",
                deprecated_key="ASTER_FUTURES_TESTNET_ED25519_API_SECRET",
            )

    if environment == AsterEnvironment.DEMO:
        return get_env_key("ASTER_DEMO_API_SECRET")

    _resolve = _get_credential if account_type.is_spot_or_margin else _get_credential_soft
    return _resolve(
        standard_key="ASTER_API_SECRET",
        deprecated_key="ASTER_ED25519_API_SECRET",  # gitleaks:allow
    )
