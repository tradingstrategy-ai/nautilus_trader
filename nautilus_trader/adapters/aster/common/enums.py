...
# -------------------------------------------------------------------------------------------------
"""
Defines Aster common enums.

References
----------
https://docs.asterdex.com/spot/en/#public-api-definitions
...
@unique
class AsterKeyType(Enum):
    """
    Represents an Aster private key cryptographic algorithm type.
    """
...
@unique
class AsterEnvironment(Enum):
    """
    Represents an Aster trading environment.
    """
    ...
    def is_live(self) -> bool:
        return self == AsterEnvironment.LIVE
...
@unique
class AsterFuturesPositionSide(Enum):
    """
    Represents a Aster Futures position side.
    """
...
@unique
class AsterRateLimitType(Enum):
    """
    Represents an Aster rate limit type.
    """
...
@unique
class AsterRateLimitInterval(Enum):
    """
    Represents an Aster rate limit interval.
    """
...
@unique
class AsterKlineInterval(Enum):
    """
    Represents an Aster kline chart interval.
    """
...
@unique
class AsterExchangeFilterType(Enum):
    """
    Represents an Aster exchange filter type.
    """
...
@unique
class AsterSymbolFilterType(Enum):
    """
    Represents an Aster symbol filter type.
    """
...
@unique
class AsterAccountType(Enum):
    """
    Represents an Aster account type.
    """
    ...
    @property
    def is_spot(self):
        return self == AsterAccountType.SPOT

    @property
    def is_margin(self):
        return self in (
            AsterAccountType.MARGIN,
            AsterAccountType.ISOLATED_MARGIN,
        )

    @property
    def is_spot_or_margin(self):
        return self in (
            AsterAccountType.SPOT,
            AsterAccountType.MARGIN,
            AsterAccountType.ISOLATED_MARGIN,
        )
    ...
    def is_futures(self) -> bool:
        return self in (
            AsterAccountType.USDT_FUTURES,
            AsterAccountType.COIN_FUTURES,
        )


@unique
class AsterOrderSide(Enum):
    """
    Represents a Aster order side.
    """
...
@unique
class AsterExecutionType(Enum):
    """
    Represents a Aster execution type.
    """
...
@unique
class AsterOrderStatus(Enum):
    """
    Represents a Aster order status.
    """
...
@unique
class AsterTimeInForce(Enum):
    """
    Represents a Aster order time in force.
    """
...
@unique
class AsterOrderType(Enum):
    """
    Represents a Aster order type.
    """
...
@unique
class AsterSecurityType(Enum):
    """
    Represents an Aster endpoint security type.
    """
...
@unique
class AsterNewOrderRespType(Enum):
    """
    Represents an Aster newOrderRespType.
    """
...
@unique
class AsterErrorCode(Enum):
    """
    Represents an Aster error code (covers futures).
    """
    ...
    PRICE_HIGHTER_THAN_MULTIPLIER_UP = -4016  # Aster's official typo (should be HIGHER)
...
class AsterEnumParser:
    """
    Provides common parsing methods for enums used by the 'Aster' exchange.
    ...
    def __init__(self) -> None:
        ...
        self.ext_to_int_status = {
            AsterOrderStatus.NEW: OrderStatus.ACCEPTED,
            AsterOrderStatus.CANCELED: OrderStatus.CANCELED,
            AsterOrderStatus.PENDING_CANCEL: OrderStatus.PENDING_CANCEL,
            AsterOrderStatus.REJECTED: OrderStatus.REJECTED,
            AsterOrderStatus.PARTIALLY_FILLED: OrderStatus.PARTIALLY_FILLED,
            AsterOrderStatus.FILLED: OrderStatus.FILLED,
            AsterOrderStatus.NEW_ADL: OrderStatus.FILLED,
            AsterOrderStatus.NEW_INSURANCE: OrderStatus.FILLED,
            AsterOrderStatus.EXPIRED: OrderStatus.EXPIRED,
            AsterOrderStatus.EXPIRED_IN_MATCH: OrderStatus.CANCELED,  # Canceled due self-trade prevention (STP)
            AsterOrderStatus.TRIGGERING: OrderStatus.ACCEPTED,  # Algo order forwarding to matching engine
            AsterOrderStatus.TRIGGERED: OrderStatus.ACCEPTED,  # Algo order placed in matching engine
            # FINISHED intentionally omitted - requires aq field to determine filled vs canceled
        }

        self.ext_to_int_order_side = {
            AsterOrderSide.BUY: OrderSide.BUY,
            AsterOrderSide.SELL: OrderSide.SELL,
        }
        ...
        self.ext_to_int_time_in_force = {
            ...
            AsterTimeInForce.GTC: TimeInForce.GTC,
            AsterTimeInForce.GTX: TimeInForce.GTC,  # Convert GTX to GTC
            AsterTimeInForce.GTE_GTC: TimeInForce.GTC,  # Undocumented
            AsterTimeInForce.IOC: TimeInForce.IOC,
            AsterTimeInForce.GTD: TimeInForce.GTD,
        }
        self.int_to_ext_time_in_force = {
            TimeInForce.GTC: AsterTimeInForce.GTC,
            TimeInForce.GTD: AsterTimeInForce.GTD,
            TimeInForce.FOK: AsterTimeInForce.FOK,
            TimeInForce.IOC: AsterTimeInForce.IOC,
        }

    def parse_aster_order_side(self, order_side: AsterOrderSide) -> OrderSide:
        try:
            return self.ext_to_int_order_side[order_side]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized Aster order side, was {order_side}",  # pragma: no cover
            )

    def parse_internal_order_side(self, order_side: OrderSide) -> AsterOrderSide:
        try:
    ...
    def parse_aster_time_in_force(self, time_in_force: AsterTimeInForce) -> TimeInForce:
        try:
            return self.ext_to_int_time_in_force[time_in_force]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized Aster time in force, was {time_in_force}",  # pragma: no cover
            )

    def parse_internal_time_in_force(self, time_in_force: TimeInForce) -> AsterTimeInForce:
        try:
    ...
    def parse_aster_order_status(self, order_status: AsterOrderStatus) -> OrderStatus:
        try:
            return self.ext_to_int_status[order_status]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized aster order status, was {order_status}",  # pragma: no cover
            )
    ...
    def parse_aster_bar_agg(self, bar_agg: str) -> BarAggregation:
        try:
            return self.ext_to_int_bar_agg[bar_agg]
        except KeyError:
            raise RuntimeError(  # pragma: no cover (design-time error)
                f"unrecognized Aster kline resolution, was {bar_agg}",
            )
    ...
    def parse_aster_kline_interval_to_bar_spec(
        self,
        kline_interval: AsterKlineInterval,
    ) -> BarSpecification:
        step = kline_interval.value[:-1]
        aster_bar_agg = kline_interval.value[-1]
        return BarSpecification(
            step=int(step),
            aggregation=self.parse_aster_bar_agg(aster_bar_agg),
    ...
    def parse_aster_trigger_type(self, trigger_type: str) -> TriggerType:
        ...
        raise NotImplementedError(  # pragma: no cover (design-time error)
            "Cannot parse aster trigger type (not implemented).",  # pragma: no cover
        )

    def parse_position_id_to_aster_futures_position_side(
        self,
        position_id: PositionId,
    ) -> AsterFuturesPositionSide:
        if position_id.value.endswith("LONG"):  # Position Long
            return AsterFuturesPositionSide.LONG
        elif position_id.value.endswith("SHORT"):  # Position Short
            return AsterFuturesPositionSide.SHORT
