...
class AsterWebSocketClient:
    ...
    References
    ----------
    https://docs.asterdex.com/spot/en/#websocket-market-streams
    ...
    async def _connect_client(self, client_id: int, streams: list[str]) -> None:
        ...
        # Aster expects at least one stream for the initial connection
    ...
    async def subscribe_agg_trades(self, symbol: str) -> None:
        ...
        """
        stream = f"{AsterSymbol(symbol).lower()}@aggTrade"
    ...
    async def subscribe_trades(self, symbol: str) -> None:
        ...
        """
        stream = f"{AsterSymbol(symbol).lower()}@trade"
        await self._subscribe(stream)
    ...
    async def subscribe_bars(
        ...
        """
        stream = f"{AsterSymbol(symbol).lower()}@kline_{interval}"
        await self._subscribe(stream)
    ...
    async def subscribe_mini_ticker(
        ...
        if symbol is None:
            ...
            stream = f"{AsterSymbol(symbol).lower()}@miniTicker"
        await self._subscribe(stream)
    ...
    async def subscribe_ticker(
        ...
        if symbol is None:
            ...
            stream = f"{AsterSymbol(symbol).lower()}@ticker"
        await self._subscribe(stream)
    ...
    async def subscribe_book_ticker(
        ...
        if symbol is None:
            ...
            stream = f"{AsterSymbol(symbol).lower()}@bookTicker"
        await self._subscribe(stream)
    ...
    async def subscribe_partial_book_depth(
        ...
        """
        stream = f"{AsterSymbol(symbol).lower()}@depth{depth}@{speed}ms"
        await self._subscribe(stream)
    ...
    async def subscribe_diff_book_depth(
        ...
        """
        stream = f"{AsterSymbol(symbol).lower()}@depth@{speed}ms"
        await self._subscribe(stream)
    ...
    async def subscribe_mark_price(
        ...
        if symbol is None:
            ...
            stream = f"{AsterSymbol(symbol).lower()}@markPrice"
    ...
    async def unsubscribe_mark_price(
        ...
        if symbol is None:
            ...
            stream = f"{AsterSymbol(symbol).lower()}@markPrice@{int(speed / 1000)}s"
        await self._unsubscribe(stream)
