...
# -------------------------------------------------------------------------------------------------
"""
Aster WebSocket API client for user data streams.
...
Futures + HMAC uses REST API for listenKey management (Aster Futures WS API
...
from nautilus_trader.adapters.aster.common.credentials import extract_ed25519_private_key
...
from nautilus_trader.adapters.aster.http.user import AsterUserDataHttpAPI
...
class AsterUserDataWebSocketClient:
    """
    Provides a Aster WebSocket API client for user data streams.
    ...
    api_key : str
        The Aster API key.
    api_secret : str
        The Aster API secret (HMAC string or Ed25519 base64/PEM key).
    ...
    stream_base_url : str, optional
        The base URL for the futures stream connection (e.g. wss://fstream.asterdex.com).
    ...
    http_client : AsterHttpClient, optional
    ...
    account_type : AsterAccountType, optional
    ...
    def __init__(
        ...
        http_client: AsterHttpClient | None = None,
        account_type: AsterAccountType | None = None,
    ) -> None:
        self._clock = clock
        ...
        if http_client is not None and account_type is not None:
            self._http_user: AsterUserDataHttpAPI | None = AsterUserDataHttpAPI(
                client=http_client,
    ...
    def _handle_message(self, raw: bytes) -> None:
        ...
        # Normalize to string since Aster may return numeric IDs
        msg_id = msg.get("id")
