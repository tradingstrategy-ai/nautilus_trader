...
class AsterFuturesBalanceInfo(msgspec.Struct, frozen=True):
    """
    HTTP response 'inner struct' from Aster Futures GET /fapi/v2/account (HMAC
    SHA256).
    """
...
class AsterFuturesAccountInfo(msgspec.Struct, kw_only=True, frozen=True):
    """
    HTTP response from Aster Futures GET /fapi/v2/account (HMAC SHA256).
    """
    ...
    assets: list[AsterFuturesBalanceInfo]
...
class AsterFuturesPositionRisk(msgspec.Struct, kw_only=True, frozen=True):
    """
    HTTP response from Aster Futures GET /fapi/v3/positionRisk (HMAC SHA256).
    ...
    symbol: str
    positionSide: AsterFuturesPositionSide
    positionAmt: str
    entryPrice: str
    markPrice: str
    ...
    def parse_to_position_status_report(
        ...
        enum_parser: AsterFuturesEnumParser,
        report_id: UUID4,
        ts_init: int,
        ...
        if self.positionSide in (
            AsterFuturesPositionSide.LONG,
            AsterFuturesPositionSide.SHORT,
        ):
            position_side = (
                PositionSide.LONG
                if self.positionSide == AsterFuturesPositionSide.LONG
...
class AsterFuturesDualSidePosition(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures GET /fapi/v1/positionSide/dual (HMAC SHA256).
    """

    dualSidePosition: bool


class AsterFuturesFeeRates(msgspec.Struct, frozen=True):
    """
    Represents a Aster Futures fee tier.
...
class AsterFuturesLeverage(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures POST /fapi/v1/leverage.
    """
...
class AsterFuturesSymbolConfig(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures GET /fapi/v1/symbolConfig.
...
class AsterFuturesMarginTypeResponse(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures `POST /fapi/v1/marginType`.
    """
...
class AsterFuturesAlgoOrder(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures `POST /fapi/v1/algoOrder` and `GET
    ...
    def parse_to_order_status_report(
        ...
        if self.workingType is not None:
            trigger_type = enum_parser.parse_aster_trigger_type(self.workingType)
        elif self.triggerPrice and Decimal(self.triggerPrice) > 0:
            trigger_type = TriggerType.LAST_PRICE

        # Aster sends callbackRate in percent (e.g., 1.0 = 1%), convert to basis points
        ...
        aster_order_type = AsterOrderType(self.orderType)
        order_type = enum_parser.parse_aster_order_type(aster_order_type)

        aster_order_side = AsterOrderSide(self.side)
        order_side = enum_parser.parse_aster_order_side(aster_order_side)
        ...
        if self.timeInForce:
            aster_tif = AsterTimeInForce(self.timeInForce)
            time_in_force = enum_parser.parse_aster_time_in_force(aster_tif)
...
class AsterFuturesAlgoOrderCancelResponse(msgspec.Struct, frozen=True):
    """
    HTTP response from Aster Futures `DELETE /fapi/v1/algoOrder`.

    References
    ----------
    https://developers.aster.com/docs/derivatives/usds-margined-futures/trade/rest-api/Cancel-Algo-Order
