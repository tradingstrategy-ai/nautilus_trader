...
from nautilus_trader.adapters.aster.common.enums import AsterOrderStatus
...
from nautilus_trader.adapters.aster.futures.enums import AsterFuturesPositionUpdateReason
from nautilus_trader.adapters.aster.futures.enums import AsterFuturesWorkingType
...
class AsterFuturesUserMsgData(msgspec.Struct, frozen=True):
    ...
    e: AsterFuturesEventType


class AsterFuturesUserMsgWrapper(msgspec.Struct, frozen=True):
    """
    Provides a wrapper for execution WebSocket messages from Aster.
    """

    data: AsterFuturesUserMsgData | None = None
    stream: str | None = None


class MarginCallPosition(msgspec.Struct, frozen=True):
    """
    Inner struct position for Aster Futures Margin Call events.
    """

    s: str  # Symbol
    ps: AsterFuturesPositionSide  # Position Side
...
class AsterFuturesMarginCallMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Margin Call events.
    """
...
class AsterFuturesBalance(msgspec.Struct, frozen=True):
    """
    Inner struct balance for Aster Futures Balance and Position update event.
    """
...
class AsterFuturesPosition(msgspec.Struct, frozen=True):
    """
    Inner struct position for Aster Futures Balance and Position update event.
    """
    ...
    ps: AsterFuturesPositionSide


class AsterFuturesAccountUpdateData(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Balance and Position Update events.
    """

    m: AsterFuturesPositionUpdateReason
    B: list[AsterFuturesBalance]
    P: list[AsterFuturesPosition]

    def parse_to_account_balances(self) -> list[AccountBalance]:
        return [balance.parse_to_account_balance() for balance in self.B]


class AsterFuturesAccountUpdateMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Balance and Position Update events.
    """
    ...
    a: AsterFuturesAccountUpdateData

    def handle_account_update(self, exec_client: AsterCommonExecutionClient):
        """
        Handle AsterFuturesAccountUpdateMsg as payload of ACCOUNT_UPDATE.
        """
...
class AsterFuturesAccountUpdateWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for Aster Futures Balance and Position Update events.
    """

    stream: str
    data: AsterFuturesAccountUpdateMsg


class AsterFuturesOrderData(msgspec.Struct, kw_only=True, frozen=True):
    """
    WebSocket message 'inner struct' for Aster Futures Order Update events.
    ...
    def parse_to_order_status_report(
        ...
        order_side = OrderSide.BUY if self.S == AsterOrderSide.BUY else OrderSide.SELL
        post_only = self.f == AsterTimeInForce.GTX
        ...
        return OrderStatusReport(
            ...
            order_type=enum_parser.parse_aster_order_type(self.o),
            time_in_force=enum_parser.parse_aster_time_in_force(self.f),
            order_status=enum_parser.parse_aster_order_status(self.X),
            expire_time=expire_time,
            price=price,
            trigger_price=trigger_price,
            trigger_type=enum_parser.parse_aster_trigger_type(self.wt.value),
    ...
    def handle_order_trade_update(  # noqa: C901 (too complex)
        self,
        exec_client: AsterCommonExecutionClient,
    ) -> None:
        """
        Handle AsterFuturesOrderData as payload of ORDER_TRADE_UPDATE event.
        """
        ...
        # Aster sends liquidation/ADL fills with x=CALCULATED and X=FILLED
        if (is_liquidation or is_adl or is_settlement) and (
            self.x == AsterExecutionType.CALCULATED
            or self.X == AsterOrderStatus.NEW_ADL
            or self.X == AsterOrderStatus.NEW_INSURANCE
            or self.X == AsterOrderStatus.FILLED
        ):
            ...
            fill_report = FillReport(
                ...
                order_side=exec_client._enum_parser.parse_aster_order_side(self.S),
        ...
        if self.x == AsterExecutionType.NEW:
            ...
            if order.has_price:
                aster_price = Price(float(self.p), price_precision)
                if aster_price != order.price:
        ...
        elif self.x == AsterExecutionType.TRADE or self.x == AsterExecutionType.CALCULATED:
            if self.x == AsterExecutionType.CALCULATED:
                exec_client._log.info(
            ...
            if Decimal(self.L) == 0:
                ...
                if self.X == AsterOrderStatus.EXPIRED:
                    ...
                    return
                elif self.X == AsterOrderStatus.CANCELED or (
                    exec_client.treat_expired_as_canceled and self.x == AsterExecutionType.EXPIRED
                ):
                    ...
                    return
                elif self.X in (AsterOrderStatus.FILLED, AsterOrderStatus.PARTIALLY_FILLED):
            ...
            exec_client.generate_order_filled(
                ...
                order_side=exec_client._enum_parser.parse_aster_order_side(self.S),
                order_type=exec_client._enum_parser.parse_aster_order_type(self.o),
                last_qty=last_qty,
                last_px=last_px,
        ...
        elif self.x == AsterExecutionType.CANCELED or (
            exec_client.treat_expired_as_canceled and self.x == AsterExecutionType.EXPIRED
        ):
        ...
        elif self.x == AsterExecutionType.AMENDMENT:
            exec_client.generate_order_updated(
                strategy_id=strategy_id,
                instrument_id=instrument_id,
                client_order_id=client_order_id,
        ...
        elif self.x == AsterExecutionType.EXPIRED:
            if order.order_type == OrderType.TRAILING_STOP_MARKET:
                exec_client.generate_order_updated(
                    strategy_id=strategy_id,
                    instrument_id=instrument_id,
        ...
        elif self.x == AsterExecutionType.REJECTED:
            due_post_only = self.f == AsterTimeInForce.GTX
        ...
        elif self.x == AsterExecutionType.TRADE_PREVENTION:
            exec_client._log.info(
...
class AsterFuturesOrderUpdateMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Order Update events.
    """

    e: str  # Event Type
    E: int  # Event Time
    T: int  # Transaction Time
    o: AsterFuturesOrderData


class AsterFuturesOrderUpdateWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for Aster Futures Order Update events.
    """

    stream: str
    data: AsterFuturesOrderUpdateMsg


class AsterFuturesTradeLiteMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Trade Lite events.
    """
    ...
    def to_order_data(self) -> AsterFuturesOrderData:
        """
        Convert TradeLite message to OrderData format.
        """
        return AsterFuturesOrderData(
            s=self.s,
            c=self.c,
            S=self.S,
            o=AsterOrderType.LIMIT,
            f=AsterTimeInForce.GTC,
            q=self.q,
            p=self.p,
            ap="0",
            x=AsterExecutionType.TRADE,
            X=AsterOrderStatus.FILLED,
            i=self.i,
            l=self.l,
            z=self.l,
            L=self.L,
            ...
            m=self.m,
            R=False,
            wt=AsterFuturesWorkingType.CONTRACT_PRICE,
            ot=AsterOrderType.LIMIT,
            ps=AsterFuturesPositionSide.BOTH,
            rp="0",
            gtd=0,
            pP=False,
            si=0,
...
class AsterFuturesTradeLiteWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for Aster Futures Trade Lite events.
    """

    stream: str
    data: AsterFuturesTradeLiteMsg


class AsterFuturesAlgoOrderData(msgspec.Struct, kw_only=True, frozen=True):
    """
    WebSocket message 'inner struct' for Aster Futures Algo Order Update events.

    References
    ----------
    https://developers.aster.com/docs/derivatives/usds-margined-futures/user-data-streams/Event-Algo-Order-Update
    ...
    o: AsterOrderType  # Order Type
    s: str  # Symbol
    S: AsterOrderSide  # Side
    ps: AsterFuturesPositionSide  # Position Side
    f: AsterTimeInForce  # Time In Force
    q: str  # Quantity
    X: AsterOrderStatus  # Algo Status (NEW, TRIGGERING, TRIGGERED, FINISHED, etc.)
    tp: str  # Trigger Price
    p: str  # Price
    wt: AsterFuturesWorkingType  # Working Type
    pm: str  # Price Match
    ...
    def handle_algo_update(
        ...
        """
        Handle AsterFuturesAlgoOrderData as payload of ALGO_UPDATE event.
        """
    ...
    def _handle_algo_status(
        ...
        if self.X == AsterOrderStatus.NEW:
            exec_client.generate_order_accepted(
                strategy_id=strategy_id,
                instrument_id=instrument_id,
                client_order_id=client_order_id,
        ...
        elif self.X == AsterOrderStatus.CANCELED:
            exec_client._triggered_algo_order_ids.discard(client_order_id)
            exec_client.generate_order_canceled(
                strategy_id=strategy_id,
                instrument_id=instrument_id,
        ...
        elif self.X == AsterOrderStatus.EXPIRED:
            exec_client._triggered_algo_order_ids.discard(client_order_id)
        ...
        elif self.X == AsterOrderStatus.REJECTED:
            exec_client._triggered_algo_order_ids.discard(client_order_id)
            exec_client.generate_order_rejected(
                strategy_id=strategy_id,
                instrument_id=instrument_id,
        ...
        elif self.X == AsterOrderStatus.TRIGGERING:
            exec_client._log.info(
                f"Algo order {client_order_id} triggering, algo_id={self.aid}, symbol={self.s}",
            )
        elif self.X == AsterOrderStatus.TRIGGERED:
        ...
        elif self.X == AsterOrderStatus.FINISHED:
    ...
    def _parse_to_order_status_report(
        ...
        if self.X == AsterOrderStatus.FINISHED:
            order_status, filled_qty, avg_px = self._resolve_finished_status()
        else:
            order_status = enum_parser.parse_aster_order_status(self.X)
            filled_qty = (
...
class AsterFuturesAlgoUpdateMsg(msgspec.Struct, frozen=True):
    """
    WebSocket message for Aster Futures Algo Order Update events.
    """

    e: str  # Event Type
    E: int  # Event Time
    T: int  # Transaction Time
    o: AsterFuturesAlgoOrderData


class AsterFuturesAlgoUpdateWrapper(msgspec.Struct, frozen=True):
    """
    WebSocket message wrapper for Aster Futures Algo Order Update events.
    """

    stream: str
    data: AsterFuturesAlgoUpdateMsg
