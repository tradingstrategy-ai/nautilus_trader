...
//! Integration tests for the Aster Futures WebSocket client using a mock server.
...
use nautilus_aster::{
    common::enums::{AsterEnvironment, AsterProductType},
    futures::websocket::streams::client::AsterFuturesWebSocketClient,
};
...
fn create_test_client(addr: &SocketAddr) -> AsterFuturesWebSocketClient {
    let ws_url = format!("ws://{addr}/ws");
    AsterFuturesWebSocketClient::new(
...
async fn test_connection_failure_invalid_url() {
    let result = AsterFuturesWebSocketClient::new(
        AsterProductType::UsdM,
        AsterEnvironment::Mainnet,
        None,
        None,
...
async fn test_default_client_creation_usdm() {
    let client = AsterFuturesWebSocketClient::new(
        AsterProductType::UsdM,
        AsterEnvironment::Mainnet,
        None,
        None,
    ...
    assert!(!client.is_active());
    assert!(client.is_closed());
    assert_eq!(client.product_type(), AsterProductType::UsdM);
}
...
async fn test_default_client_creation_coinm() {
    ...
    assert!(!client.is_active());
    assert!(client.is_closed());
    assert_eq!(client.product_type(), AsterProductType::CoinM);
}
