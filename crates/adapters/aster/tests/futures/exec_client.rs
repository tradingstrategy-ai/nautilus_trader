...
//! Integration tests for the Aster Futures execution client.
...
use nautilus_aster::{
    config::AsterExecClientConfig, futures::execution::AsterFuturesExecutionClient,
};
use nautilus_common::{
    cache::Cache,
...
fn create_test_execution_client(
    ...
    let client_id = ClientId::from("ASTER");
    ...
    let config = AsterExecClientConfig {
        trader_id,
        account_id,
    ...
    let client = AsterFuturesExecutionClient::new(core, config).unwrap();
...
async fn test_client_creation() {
    ...
    assert_eq!(client.client_id(), ClientId::from("ASTER"));
    assert_eq!(client.venue(), Venue::from("ASTER"));
...
async fn test_connect_loads_instruments_and_account() {
    ...
    add_test_account_to_cache(&cache, AccountId::from("ASTER-001"));
