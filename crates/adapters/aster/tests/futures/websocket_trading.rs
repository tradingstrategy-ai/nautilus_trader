...
//! Integration tests for the Aster Futures WebSocket Trading client using a mock server.
...
use nautilus_aster::{
    common::enums::{AsterFuturesOrderType, AsterSide, AsterTimeInForce},
    futures::{
        http::query::{
            AsterCancelOrderParamsBuilder, AsterModifyOrderParamsBuilder,
            AsterNewOrderParamsBuilder,
        },
        websocket::trading::{
...
fn create_test_client(addr: &SocketAddr) -> AsterFuturesWsTradingClient {
    let ws_url = format!("ws://{addr}/ws-fapi/v1");
    AsterFuturesWsTradingClient::new(
        Some(ws_url),
...
async fn test_connection_failure_invalid_url() {
    let mut client = AsterFuturesWsTradingClient::new(
...
async fn test_place_order_request_format() {
    ...
    assert!(matches!(
        msg,
        Some(AsterFuturesWsTradingMessage::Connected)
    ));

    let params = AsterNewOrderParamsBuilder::default()
        .symbol("BTCUSDT")
        .side(AsterSide::Buy)
...
async fn test_place_order_accepted() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::OrderAccepted {
            request_id,
            response,
        }) => {
...
async fn test_place_order_rejected() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::OrderRejected {
            request_id,
            code,
            msg,
        }) => {
...
async fn test_cancel_order_request_format() {
    ...
    let params = AsterCancelOrderParamsBuilder::default()
        .symbol("BTCUSDT")
        .order_id(12345_i64)
        .build()
        .unwrap();
...
async fn test_cancel_order_accepted() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::OrderCanceled {
            request_id,
            response,
        }) => {
            assert!(request_id.starts_with("req-"));
...
async fn test_cancel_order_rejected() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::CancelRejected {
            request_id,
            code,
            msg,
        }) => {
...
async fn test_modify_order_request_format() {
    ...
    let params = AsterModifyOrderParamsBuilder::default()
        .symbol("BTCUSDT")
        .order_id(12345_i64)
        .side(AsterSide::Buy)
        .quantity("0.002")
...
async fn test_modify_order_accepted() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::OrderModified {
            request_id,
            response,
        }) => {
            assert!(request_id.starts_with("req-"));
...
async fn test_modify_order_rejected() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::ModifyRejected {
            request_id,
            code,
            msg,
        }) => {
...
async fn test_cancel_all_orders() {
    ...
    match msg {
        Some(AsterFuturesWsTradingMessage::AllOrdersCanceled { request_id }) => {
            assert!(request_id.starts_with("req-"));
        }
...
async fn test_default_client_creation() {
    let client = AsterFuturesWsTradingClient::new(
        None,
        "test-key".to_string(),
