...
//! Integration tests for the Aster Futures data client.
...
use nautilus_aster::{
    common::enums::AsterProductType, config::AsterDataClientConfig,
    futures::AsterFuturesDataClient,
};
use nautilus_common::{
    clients::DataClient,
...
fn create_test_data_client(
    ...
    let config = AsterDataClientConfig {
        product_types: vec![AsterProductType::UsdM],
    ...
    let client =
        AsterFuturesDataClient::new(ClientId::from("ASTER"), config, AsterProductType::UsdM)
            .unwrap();
...
async fn test_client_creation() {
    ...
    assert_eq!(client.venue(), Some(Venue::from("ASTER")));
