...
//! Live market data client implementation for the Aster Futures adapter.
...
use crate::{
    common::{
        consts::{ASTER_BOOK_DEPTHS, ASTER_VENUE},
        enums::AsterProductType,
        parse::bar_spec_to_aster_interval,
        status::diff_and_emit_statuses,
        symbol::format_aster_stream_symbol,
    },
    config::AsterDataClientConfig,
    futures::{
        http::{
            client::AsterFuturesHttpClient, models::AsterOrderBook, query::AsterDepthParams,
        },
        websocket::streams::{
            client::AsterFuturesWebSocketClient,
            messages::AsterFuturesWsStreamsMessage,
            parse_data::{
...
/// Aster Futures data client for USD-M and COIN-M markets.
#[derive(Debug)]
pub struct AsterFuturesDataClient {
    clock: &'static AtomicTime,
    client_id: ClientId,
    config: AsterDataClientConfig,
    product_type: AsterProductType,
    http_client: AsterFuturesHttpClient,
    ws_client: AsterFuturesWebSocketClient,
...
impl AsterFuturesDataClient {
    /// Creates a new [`AsterFuturesDataClient`] instance.
    ///
    /// # Errors
    ///
    ...
    pub fn new(
        ...
        match product_type {
            ...
            _ => {
                anyhow::bail!(
                    "AsterFuturesDataClient requires UsdM or CoinM product type, was {product_type:?}"
                );
            }
        }
    ...
    fn handle_ws_message(
        ...
        match msg {
            AsterFuturesWsStreamsMessage::AggTrade(ref trade_msg) => {
            ...
            AsterFuturesWsStreamsMessage::Trade(ref trade_msg) => {
                if let Some(instrument) = cache.get(&trade_msg.symbol) {
            ...
            AsterFuturesWsStreamsMessage::BookTicker(ref ticker_msg) => {
            ...
            AsterFuturesWsStreamsMessage::DepthUpdate(ref depth_msg) => {
            ...
            AsterFuturesWsStreamsMessage::MarkPrice(ref mark_msg) => {
            ...
            AsterFuturesWsStreamsMessage::Kline(ref kline_msg) => {
            ...
            AsterFuturesWsStreamsMessage::ForceOrder(ref liq_msg) => {
                log::info!(
            ...
            AsterFuturesWsStreamsMessage::Ticker(ref ticker_msg) => {
                log::debug!(
                    "Ticker: {} last={} vol={}",
                    ticker_msg.symbol,
            ...
            AsterFuturesWsStreamsMessage::AccountUpdate(_)
            | AsterFuturesWsStreamsMessage::OrderUpdate(_)
            | AsterFuturesWsStreamsMessage::AlgoUpdate(_)
            | AsterFuturesWsStreamsMessage::MarginCall(_)
            | AsterFuturesWsStreamsMessage::AccountConfigUpdate(_)
            | AsterFuturesWsStreamsMessage::ListenKeyExpired => {}
            AsterFuturesWsStreamsMessage::Error(e) => {
                log::error!(
                    "Aster Futures WebSocket error: code={}, msg={}",
                    e.code,
                    e.msg
                );
            }
    ...
    async fn fetch_and_emit_snapshot_inner(
        ...
        let symbol = format_aster_stream_symbol(&instrument_id).to_uppercase();
        let params = AsterDepthParams {
            symbol,
            limit: Some(depth),
        };
        ...
        match http.depth(&params).await {
            Ok(order_book) => {
                ...
                // Validate first applicable update per Aster spec:
...
impl DataClient for AsterFuturesDataClient {
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        // Seed the status cache from the HTTP client's instruments cache
        {
            ...
            for entry in http_instruments.iter() {
                ...
                let action = match entry.value() {
                    crate::futures::http::client::AsterFuturesInstrument::UsdM(s) => {
                        MarketStatusAction::from(s.status)
                    }
                    crate::futures::http::client::AsterFuturesInstrument::CoinM(s) => s
                        .contract_status
        ...
        log::info!("Connecting to Aster Futures WebSocket...");
    ...
    fn subscribe_instruments(&mut self, _cmd: &SubscribeInstruments) -> anyhow::Result<()> {
        log::debug!(
            "subscribe_instruments: Aster Futures instruments are fetched via HTTP on connect"
        );
        Ok(())
    }
    ...
    fn subscribe_instrument(&mut self, _cmd: &SubscribeInstrument) -> anyhow::Result<()> {
        log::debug!(
            "subscribe_instrument: Aster Futures instruments are fetched via HTTP on connect"
        );
        Ok(())
    }
    ...
    fn subscribe_book_deltas(&mut self, cmd: &SubscribeBookDeltas) -> anyhow::Result<()> {
        if cmd.book_type != BookType::L2_MBP {
            anyhow::bail!("Aster Futures only supports L2_MBP order book deltas");
        }

        let instrument_id = cmd.instrument_id;
        let depth = cmd.depth.map_or(1000, |d| d.get() as u32);

        if !ASTER_BOOK_DEPTHS.contains(&depth) {
            anyhow::bail!(
                "Invalid depth {depth} for Aster Futures order book. \
                Valid values: {ASTER_BOOK_DEPTHS:?}"
            );
        }
        ...
        let stream = format!("{}@depth@0ms", format_aster_stream_symbol(&instrument_id));
    ...
    fn subscribe_quotes(&mut self, cmd: &SubscribeQuotes) -> anyhow::Result<()> {
        ...
        // Aster Futures uses bookTicker for best bid/ask
        let stream = format!(
            "{}@bookTicker",
            format_aster_stream_symbol(&instrument_id)
        );
    ...
    fn subscribe_trades(&mut self, cmd: &SubscribeTrades) -> anyhow::Result<()> {
        ...
        // Aster Futures uses aggTrade for aggregate trades
        let stream = format!("{}@aggTrade", format_aster_stream_symbol(&instrument_id));
    ...
    fn subscribe_bars(&mut self, cmd: &SubscribeBars) -> anyhow::Result<()> {
        ...
        let ws = self.ws_client.clone();
        let interval = bar_spec_to_aster_interval(bar_type.spec())?;

        let stream = format!(
            "{}@kline_{}",
            format_aster_stream_symbol(&bar_type.instrument_id()),
            interval.as_str()
        );
    ...
    fn unsubscribe_book_deltas(&mut self, cmd: &UnsubscribeBookDeltas) -> anyhow::Result<()> {
        ...
        let symbol_lower = format_aster_stream_symbol(&instrument_id);
        let streams = vec![
    ...
    fn request_trades(&self, request: RequestTrades) -> anyhow::Result<()> {
        ...
        get_runtime().spawn(async move {
            match http
                ...
                .await
                .context("failed to request trades from Aster Futures")
            {
                Ok(trades) => {
    ...
    fn request_bars(&self, request: RequestBars) -> anyhow::Result<()> {
        ...
        get_runtime().spawn(async move {
            match http
                ...
                .await
                .context("failed to request bars from Aster Futures")
            {
                Ok(bars) => {
