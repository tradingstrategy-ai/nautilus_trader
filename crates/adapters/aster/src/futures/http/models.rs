...
//! Aster Futures HTTP response models.
...
use crate::common::{
    ...
    enums::{
        AsterAlgoStatus, AsterAlgoType, AsterContractStatus, AsterFuturesOrderType,
        AsterIncomeType, AsterMarginType, AsterOrderStatus, AsterPositionSide,
        AsterPriceMatch, AsterSelfTradePreventionMode, AsterSide, AsterTimeInForce,
        AsterTradingStatus, AsterWorkingType,
    },
    models::AsterRateLimit,
};
...
impl<'de> Deserialize<'de> for AsterFuturesKline {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesUsdExchangeInfo {
    /// Server timezone.
    pub timezone: String,
    /// Server timestamp in milliseconds.
    pub server_time: i64,
    /// Rate limit definitions.
    pub rate_limits: Vec<AsterRateLimit>,
    /// Exchange-level filters.
    #[serde(default)]
    pub exchange_filters: Vec<Value>,
    /// Asset definitions.
    #[serde(default)]
    pub assets: Vec<AsterFuturesAsset>,
    /// Trading symbols.
    pub symbols: Vec<AsterFuturesUsdSymbol>,
}

/// Futures asset definition.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesAsset {
    /// Asset name.
    pub asset: Ustr,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesUsdSymbol {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesCoinExchangeInfo {
    /// Server timezone.
    pub timezone: String,
    /// Server timestamp in milliseconds.
    pub server_time: i64,
    ...
    pub exchange_filters: Vec<Value>,
    /// Trading symbols.
    pub symbols: Vec<AsterFuturesCoinSymbol>,
}

/// COIN-M Futures symbol definition.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesCoinSymbol {
    ...
    /// Trading status.
    #[serde(default)]
    pub contract_status: Option<AsterContractStatus>,
    /// Contract size.
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesTicker24hr {
    /// Symbol name.
    pub symbol: Ustr,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesMarkPrice {
    /// Symbol name.
    pub symbol: Ustr,
    /// Mark price.
...
pub struct AsterAccountPosition {
    ...
    #[serde(default)]
    pub position_side: Option<AsterPositionSide>,
    /// Position amount.
    #[serde(default)]
...
pub struct AsterPositionRisk {
    ...
    #[serde(default)]
    pub margin_type: Option<AsterMarginType>,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesAccountInfo {
    ...
    /// Account balances.
    #[serde(default)]
    pub assets: Vec<AsterFuturesBalance>,
    /// Account positions.
    #[serde(default)]
    pub positions: Vec<AsterAccountPosition>,
}

impl AsterFuturesAccountInfo {
    /// Converts this Aster account info to a Nautilus [`AccountState`].
    ///
    /// # Errors
    ///
            ...
            (initial_margin_dec, maint_margin_dec)
        {
            ...
            if has_margin {
                ...
                let margin_instrument_id =
                    InstrumentId::new(Symbol::new("ACCOUNT"), Venue::new("ASTER"));
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterHedgeModeResponse {
...
pub struct AsterFuturesOrder {
    ...
    /// Order type.
    #[serde(rename = "type")]
    pub order_type: AsterFuturesOrderType,
    /// Original order type.
    #[serde(default)]
    pub orig_type: Option<AsterFuturesOrderType>,
    /// Order side (BUY/SELL).
    pub side: AsterSide,
    /// Position side (BOTH/LONG/SHORT).
    #[serde(default)]
    ...
    #[serde(default)]
    pub working_type: Option<AsterWorkingType>,
    ...
    /// Price match mode.
    #[serde(default)]
    pub price_match: Option<AsterPriceMatch>,
    /// Self-trade prevention mode.
    #[serde(default)]
    pub self_trade_prevention_mode: Option<AsterSelfTradePreventionMode>,
    /// Last update time.
    #[serde(default)]
    pub update_time: Option<i64>,
...
impl AsterFuturesOrder {
    /// Converts this Aster order to a Nautilus [`OrderStatusReport`].
    ///
    /// # Errors
    ///
    ...
    pub fn to_order_status_report(
        ...
        let order_side = match self.side {
            AsterSide::Buy => OrderSide::Buy,
            AsterSide::Sell => OrderSide::Sell,
        };
...
impl AsterFuturesOrderType {
    /// Returns whether this order type is post-only.
    #[must_use]
    pub fn is_post_only(&self) -> bool {
        false // Aster Futures doesn't have a dedicated post-only type
    }
...
impl AsterUserTrade {
    /// Converts this Aster trade to a Nautilus [`FillReport`].
    ///
    /// # Errors
    ///
...
/// Algo order response from Aster Futures Algo Service API.
///
/// Algo orders are conditional orders (STOP_MARKET, STOP_LIMIT, TAKE_PROFIT,
/// TAKE_PROFIT_MARKET, TRAILING_STOP_MARKET) that are managed by Aster's
/// Algo Service rather than the traditional order matching engine.
///
/// # References
///
/// - <https://developers.asterdex.com/docs/derivatives/usds-margined-futures/trade/rest-api/New-Algo-Order>
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesAlgoOrder {
    /// Unique algo order ID assigned by Aster.
    pub algo_id: i64,
    /// Client-specified algo order ID for idempotency.
    pub client_algo_id: String,
    /// Algo type (currently only `Conditional` is supported).
    pub algo_type: AsterAlgoType,
    ...
    /// Time in force.
    #[serde(default)]
    pub time_in_force: Option<AsterTimeInForce>,
    /// Order quantity.
    #[serde(default)]
    pub quantity: Option<String>,
    /// Algo order status.
    #[serde(default)]
    pub algo_status: Option<AsterAlgoStatus>,
...
impl AsterFuturesAlgoOrder {
    /// Converts this Aster algo order to a Nautilus [`OrderStatusReport`].
    ///
    /// # Errors
    ///
    /// Returns an error if quantity parsing fails.
    ...
    fn parse_order_status(&self) -> OrderStatus {
        match self.algo_status {
            Some(AsterAlgoStatus::New) => OrderStatus::Accepted,
            Some(AsterAlgoStatus::Triggering) => OrderStatus::Accepted,
            Some(AsterAlgoStatus::Triggered) => OrderStatus::Accepted,
            Some(AsterAlgoStatus::Finished) => {
            ...
            Some(AsterAlgoStatus::Canceled) => OrderStatus::Canceled,
            Some(AsterAlgoStatus::Expired) => OrderStatus::Expired,
            Some(AsterAlgoStatus::Rejected) => OrderStatus::Rejected,
            Some(AsterAlgoStatus::Unknown) | None => OrderStatus::Initialized,
        }
    }
}

/// Cancel response for algo orders from Aster Futures Algo Service API.
#[derive(Clone, Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AsterFuturesAlgoOrderCancelResponse {
...
mod tests {
    ...
    fn test_parse_account_info_v2() {
        ...
        let account: AsterFuturesAccountInfo =
    ...
    fn test_account_info_to_account_state_with_margins() {
        ...
        assert_eq!(margin.instrument_id.venue.as_str(), "ASTER");
    ...
    fn test_parse_balance_with_v1_field() {
        ...
        let balances: Vec<AsterFuturesBalance> =
    ...
    fn test_parse_balance_with_v2_field() {
        ...
        let balance: AsterFuturesBalance =
            serde_json::from_str(json).expect("Failed to parse balance");
    ...
    fn test_parse_order() {
        ...
        assert_eq!(order.status, AsterOrderStatus::New);
        assert_eq!(order.time_in_force, AsterTimeInForce::Gtc);
        assert_eq!(order.side, AsterSide::Buy);
        assert_eq!(order.order_type, AsterFuturesOrderType::Limit);
        assert_eq!(order.price_match, Some(AsterPriceMatch::None));
        assert_eq!(
            order.self_trade_prevention_mode,
            Some(AsterSelfTradePreventionMode::None)
        );
    }
    ...
    fn test_parse_hedge_mode_response() {
        ...
        let response: AsterHedgeModeResponse =
    ...
    fn test_parse_leverage_response() {
        ...
        let response: AsterLeverageResponse =
    ...
    fn test_parse_account_position() {
        ...
        let position: AsterAccountPosition =
        ...
        assert_eq!(position.position_side, Some(AsterPositionSide::Long));
    }
    ...
    fn test_parse_algo_order() {
        ...
        let order: AsterFuturesAlgoOrder =
        ...
        assert_eq!(order.algo_type, AsterAlgoType::Conditional);
        assert_eq!(order.order_type, AsterFuturesOrderType::StopMarket);
        assert_eq!(order.symbol.as_str(), "BTCUSDT");
        assert_eq!(order.side, AsterSide::Buy);
        assert_eq!(order.algo_status, Some(AsterAlgoStatus::New));
    ...
    fn test_parse_algo_order_triggered() {
        ...
        assert_eq!(order.algo_status, Some(AsterAlgoStatus::Triggered));
        assert_eq!(order.order_type, AsterFuturesOrderType::TakeProfit);
    ...
    fn test_parse_algo_order_cancel_response() {
        ...
        let response: AsterFuturesAlgoOrderCancelResponse =
    ...
    fn test_order_to_report_decodes_broker_id() {
        ...
        let order: AsterFuturesOrder = serde_json::from_str(json).unwrap();
        let account_id = AccountId::from("ASTER-FUTURES-001");
        let instrument_id = InstrumentId::from("BTCUSDT-PERP.ASTER");
        let ts_init = UnixNanos::from(1_000_000_000u64);
    ...
    fn test_algo_order_to_report_decodes_broker_id() {
        ...
        let order: AsterFuturesAlgoOrder = serde_json::from_str(json).unwrap();
        let account_id = AccountId::from("ASTER-FUTURES-001");
        let instrument_id = InstrumentId::from("BTCUSDT-PERP.ASTER");
        let ts_init = UnixNanos::from(1_000_000_000u64);
    ...
    #[rstest]
    #[case(AsterOrderStatus::Expired, false, OrderStatus::Expired)]
    #[case(AsterOrderStatus::Expired, true, OrderStatus::Canceled)]
    #[case(AsterOrderStatus::ExpiredInMatch, false, OrderStatus::Expired)]
    #[case(AsterOrderStatus::ExpiredInMatch, true, OrderStatus::Canceled)]
    fn test_to_nautilus_order_status_expired_respects_treat_as_canceled(
        #[case] status: AsterOrderStatus,
