...
//! Aster Futures WebSocket client for JSON market data streams.
//!
//! ## Connection Details
//!
//! - USD-M Endpoint: `wss://fstream.asterdex.com/ws`
//! - COIN-M Endpoint: `wss://dstream.asterdex.com/ws`
...
use super::{
    ...
    handler::AsterFuturesDataWsFeedHandler,
    messages::{AsterFuturesWsStreamsCommand, AsterFuturesWsStreamsMessage},
};
use crate::common::{
    consts::{
        ASTER_API_KEY_HEADER, ASTER_RATE_LIMIT_KEY_SUBSCRIPTION, ASTER_WS_CONNECTION_QUOTA,
        ASTER_WS_SUBSCRIPTION_QUOTA,
    },
...
struct ConnectionSlot {
    cmd_tx: tokio::sync::mpsc::UnboundedSender<AsterFuturesWsStreamsCommand>,
    streams: Vec<String>,
...
#[derive(Clone)]
pub struct AsterFuturesWebSocketClient {
    url: String,
    ...
    out_rx:
        Arc<Mutex<Option<tokio::sync::mpsc::UnboundedReceiver<AsterFuturesWsStreamsMessage>>>>,
    request_id_counter: Arc<AtomicU64>,
    instruments_cache: Arc<AtomicMap<Ustr, InstrumentAny>>,
}

impl Debug for AsterFuturesWebSocketClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(AsterFuturesWebSocketClient))
            .field("url", &self.url)
...
impl AsterFuturesWebSocketClient {
    /// Creates a new [`AsterFuturesWebSocketClient`] instance.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    pub fn new_with_local_addr(
        ...
        match product_type {
            ...
            _ => {
                anyhow::bail!(
                    "AsterFuturesWebSocketClient requires UsdM or CoinM product type, was {product_type:?}"
                );
            }
        }
    ...
    #[allow(clippy::missing_panics_doc, reason = "mutex poisoning is not expected")]
    pub async fn connect(&mut self) -> AsterWsResult<()> {
        ...
        *self.out_rx.lock().expect("out_rx lock poisoned") = Some(out_rx);
        ...
        log::info!(
            "Connected to Aster Futures stream pool: url={}, product_type={:?}",
            self.url,
            self.product_type
        );
        Ok(())
    ...
    #[allow(clippy::missing_panics_doc, reason = "mutex poisoning is not expected")]
    pub async fn close(&mut self) -> AsterWsResult<()> {
        ...
        for slot in slots {
            ...
            let _ = slot.cmd_tx.send(AsterFuturesWsStreamsCommand::Disconnect);
        ...
        *self.out_rx.lock().expect("out_rx lock poisoned") = None;
        ...
        log::info!("Disconnected from Aster Futures stream pool");
        Ok(())
    }
    ...
    #[allow(clippy::missing_panics_doc, reason = "mutex poisoning is not expected")]
    pub async fn subscribe(&self, streams: Vec<String>) -> AsterWsResult<()> {
        ...
        for stream in &new_streams {
            let slot_idx = slot_counts
                ...
                .ok_or_else(|| {
                    ...
                    AsterWsError::ClientError(format!(
        ...
        for (slot_idx, batch) in &slot_batches {
            slots[*slot_idx]
                .cmd_tx
                .send(AsterFuturesWsStreamsCommand::Subscribe {
                    streams: batch.clone(),
                })
                .map_err(|e| {
                    AsterWsError::ClientError(format!(
    ...
    #[allow(clippy::missing_panics_doc, reason = "mutex poisoning is not expected")]
    pub async fn unsubscribe(&self, streams: Vec<String>) -> AsterWsResult<()> {
        let mut slots = self.slots.lock().expect("slots lock poisoned");
        let mut slot_batches: Vec<(usize, Vec<String>)> = Vec::new();
        ...
        for (slot_idx, batch) in &slot_batches {
            slots[*slot_idx]
                .cmd_tx
                .send(AsterFuturesWsStreamsCommand::Unsubscribe {
                    streams: batch.clone(),
                })
                .map_err(|e| {
                    AsterWsError::ClientError(format!(
    ...
    pub fn stream(&self) -> impl Stream<Item = AsterFuturesWsStreamsMessage> + 'static {
    ...
    async fn create_connection(&self) -> AsterWsResult<ConnectionSlot> {
        let out_tx = self
            .out_tx
            .lock()
            ...
            .clone()
            .ok_or_else(|| {
                AsterWsError::ClientError("Output channel not initialized".to_string())
            })?;
        ...
        let keyed_quotas = vec![(
            ASTER_RATE_LIMIT_KEY_SUBSCRIPTION[0].as_str().to_string(),
            *ASTER_WS_SUBSCRIPTION_QUOTA,
        )];
        ...
        let client = WebSocketClient::connect(
            ...
            None,
            keyed_quotas,
            Some(*ASTER_WS_CONNECTION_QUOTA),
        )
        .await
        .map_err(|e| AsterWsError::NetworkError(e.to_string()))?;
        ...
        let mut handler = AsterFuturesDataWsFeedHandler::new(
            self.signal.clone(),
            cmd_rx,
            bytes_rx,
            out_tx.clone(),
        ...
        cmd_tx
            .send(AsterFuturesWsStreamsCommand::SetClient(client))
            .map_err(|e| AsterWsError::ClientError(format!("Failed to set client: {e}")))?;
        ...
        let handler_task = get_runtime().spawn(async move {
            loop {
                tokio::select! {
                    ...
                    result = handler.next() => {
                        match result {
                            Some(AsterFuturesWsStreamsMessage::Reconnected) => {
                                ...
                                if !streams.is_empty()
                                    && let Err(e) = resubscribe_tx.send(AsterFuturesWsStreamsCommand::Subscribe { streams }) {
                                        log::error!("Failed to resubscribe after reconnect: {e}");
                                    }

                                if out_tx.send(AsterFuturesWsStreamsMessage::Reconnected).is_err() {
