...
//! Parsing utilities for Aster Futures WebSocket JSON messages.
...
use super::{
    error::{AsterWsError, AsterWsResult},
    messages::{
        AsterFuturesAggTradeMsg, AsterFuturesBookTickerMsg, AsterFuturesDepthUpdateMsg,
        AsterFuturesKlineMsg, AsterFuturesMarkPriceMsg, AsterFuturesTradeMsg,
    },
};
use crate::common::enums::{AsterKlineInterval, AsterWsEventType};
...
pub fn parse_agg_trade(
    msg: &AsterFuturesAggTradeMsg,
    instrument: &InstrumentAny,
    ts_init: UnixNanos,
) -> AsterWsResult<TradeTick> {
    ...
    let price = msg
        .price
        .parse::<f64>()
        .map_err(|e| AsterWsError::ParseError(e.to_string()))?;
    let size = msg
        .quantity
        .parse::<f64>()
        .map_err(|e| AsterWsError::ParseError(e.to_string()))?;
...
pub fn parse_book_ticker(
    msg: &AsterFuturesBookTickerMsg,
    instrument: &InstrumentAny,
    ts_init: UnixNanos,
) -> AsterWsResult<QuoteTick> {
    let instrument_id = instrument.id();
    let price_precision = instrument.price_precision();
    let size_precision = instrument.size_precision();
...
pub fn parse_depth_update(
    msg: &AsterFuturesDepthUpdateMsg,
    instrument: &InstrumentAny,
    ts_init: UnixNanos,
) -> AsterWsResult<OrderBookDeltas> {
    let instrument_id = instrument.id();
    let price_precision = instrument.price_precision();
    let size_precision = instrument.size_precision();
...
pub fn parse_mark_price(
    msg: &AsterFuturesMarkPriceMsg,
    instrument: &InstrumentAny,
    ts_init: UnixNanos,
) -> AsterWsResult<(MarkPriceUpdate, IndexPriceUpdate, FundingRateUpdate)> {
    let instrument_id = instrument.id();
    let price_precision = instrument.price_precision();
    ...
    let funding_update = FundingRateUpdate::new(
        ...
        None, // Aster does not provide the funding interval through WebSocket API
        next_funding_ns,
        ts_event,
        ts_init,
    );
...
/// Returns an error if parsing fails.
pub fn parse_kline(
    msg: &AsterFuturesKlineMsg,
    instrument: &InstrumentAny,
    ts_init: UnixNanos,
) -> AsterWsResult<Option<Bar>> {
...
pub fn extract_event_type(json: &serde_json::Value) -> Option<AsterWsEventType> {
    json.get("e")
...
mod tests {
    ...
    use crate::{
        common::{
            enums::{AsterOrderStatus, AsterSide, AsterTradingStatus},
            parse::parse_usdm_instrument,
            testing::{load_fixture_string, load_json_fixture},
        },
        futures::{
            http::models::AsterFuturesUsdSymbol,
            websocket::streams::messages::{AsterFuturesLiquidationMsg, AsterFuturesTickerMsg},
        },
    };
    ...
    fn sample_futures_symbol() -> AsterFuturesUsdSymbol {
    ...
    fn test_parse_agg_trade() {
        ...
        let msg: AsterFuturesAggTradeMsg = load_market_fixture("agg_trade_stream.json");
    ...
    fn test_parse_trade() {
        let instrument = sample_instrument();
        let msg: AsterFuturesTradeMsg = load_market_fixture("trade_stream.json");
        let ts_init = UnixNanos::from(1_700_000_001_000_000_000u64);
    ...
    fn test_parse_book_ticker() {
        let instrument = sample_instrument();
        let msg: AsterFuturesBookTickerMsg = load_market_fixture("book_ticker_stream.json");
        let ts_init = UnixNanos::from(1_700_000_001_000_000_000u64);
    ...
    fn test_parse_depth_update() {
        let instrument = sample_instrument();
        let msg: AsterFuturesDepthUpdateMsg = load_market_fixture("depth_update_stream.json");
        let ts_init = UnixNanos::from(1_700_000_001_000_000_000u64);
    ...
    fn test_parse_mark_price() {
        let instrument = sample_instrument();
        let msg: AsterFuturesMarkPriceMsg = load_market_fixture("mark_price_stream.json");
        let ts_init = UnixNanos::from(1_700_000_001_000_000_000u64);
    ...
    fn test_parse_kline_closed() {
        let instrument = sample_instrument();
        let msg: AsterFuturesKlineMsg = load_market_fixture("kline_stream_closed.json");
        let ts_init = UnixNanos::from(1_700_000_001_000_000_000u64);
    ...
    fn test_parse_kline_open_returns_none() {
        let instrument = sample_instrument();
        let msg: AsterFuturesKlineMsg = load_market_fixture("kline_stream_open.json");
    ...
    fn test_deserialize_liquidation_msg() {
        let msg: AsterFuturesLiquidationMsg = load_market_fixture("liquidation_stream.json");
        ...
        assert_eq!(msg.order.side, AsterSide::Sell);
        assert_eq!(msg.order.original_qty, "0.014");
        assert_eq!(msg.order.average_price, "9910.12345678");
        assert_eq!(msg.order.status, AsterOrderStatus::Filled);
    ...
    fn test_deserialize_ticker_msg() {
        let msg: AsterFuturesTickerMsg = load_market_fixture("ticker_stream.json");
    ...
    fn test_extract_event_type() {
        ...
        assert_eq!(event_type, Some(AsterWsEventType::MarkPriceUpdate));
    }
    ...
    fn test_extract_event_type_force_order() {
        ...
        assert_eq!(event_type, Some(AsterWsEventType::ForceOrder));
    }
    ...
    fn test_extract_event_type_ticker() {
        ...
        assert_eq!(event_type, Some(AsterWsEventType::Ticker24Hr));
    }
}
