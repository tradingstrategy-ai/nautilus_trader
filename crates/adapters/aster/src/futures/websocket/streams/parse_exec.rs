...
//! Parse functions for converting Aster Futures venue types to Nautilus reports.
//!
...
use super::messages::{
    AlgoOrderUpdateData, AsterFuturesAccountUpdateMsg, AsterFuturesOrderUpdateMsg,
    OrderUpdateData,
};
use crate::common::{
    ...
    enums::{
        AsterAlgoStatus, AsterFuturesOrderType, AsterOrderStatus, AsterSide,
        AsterTimeInForce,
    },
};

/// Converts a Aster Futures order update to a Nautilus order status report.
///
/// # Errors
///
...
pub fn parse_futures_order_update_to_order_status(
    ...
    report.post_only = order.order_type == AsterFuturesOrderType::Limit
        && order.time_in_force == AsterTimeInForce::Gtx;
    ...
    if let Some(ref cr) = order.callback_rate {
        ...
        if rate > 0.0 {
            // Aster callbackRate is percentage (1 = 1%), convert to basis points (100 = 1%)
...
/// Resolves the commission for a Aster fill event.
///
...
/// Converts a Aster Futures order update (Trade type) to a Nautilus fill report.
///
/// # Errors
///
/// Returns an error if report construction fails.
...
/// Converts a Aster Futures algo order update to a Nautilus order status report.
///
...
pub fn parse_futures_algo_update_to_order_status(
    ...
    let order_status = match algo_data.algo_status {
        AsterAlgoStatus::Canceled | AsterAlgoStatus::Expired => OrderStatus::Canceled,
        AsterAlgoStatus::Rejected => OrderStatus::Rejected,
        _ => return None,
    };
...
/// Converts a Aster Futures account update to a Nautilus account state.
pub fn parse_futures_account_update(
    msg: &AsterFuturesAccountUpdateMsg,
    account_id: AccountId,
    ts_init: UnixNanos,
) -> Option<AccountState> {
    let ts_event = UnixNanos::from_millis(msg.event_time as u64);
...
fn parse_side(side: AsterSide) -> OrderSide {
    match side {
...
fn parse_order_status(status: AsterOrderStatus, treat_expired_as_canceled: bool) -> OrderStatus {
    match status {
        AsterOrderStatus::New | AsterOrderStatus::PendingNew => OrderStatus::Accepted,
        AsterOrderStatus::PartiallyFilled => OrderStatus::PartiallyFilled,
        AsterOrderStatus::Filled
        | AsterOrderStatus::NewAdl
        | AsterOrderStatus::NewInsurance => OrderStatus::Filled,
        AsterOrderStatus::Canceled | AsterOrderStatus::PendingCancel => OrderStatus::Canceled,
        AsterOrderStatus::Rejected => OrderStatus::Rejected,
        AsterOrderStatus::Expired | AsterOrderStatus::ExpiredInMatch => {
        ...
        AsterOrderStatus::Unknown => OrderStatus::Accepted,
    }
}

fn parse_futures_order_type(order_type: AsterFuturesOrderType) -> OrderType {
    match order_type {
        AsterFuturesOrderType::Limit => OrderType::Limit,
        AsterFuturesOrderType::Market => OrderType::Market,
        AsterFuturesOrderType::Stop => OrderType::StopLimit,
        AsterFuturesOrderType::StopMarket => OrderType::StopMarket,
        AsterFuturesOrderType::TakeProfit => OrderType::LimitIfTouched,
        AsterFuturesOrderType::TakeProfitMarket => OrderType::MarketIfTouched,
        AsterFuturesOrderType::TrailingStopMarket => OrderType::TrailingStopMarket,
        AsterFuturesOrderType::Liquidation
        | AsterFuturesOrderType::Adl
        | AsterFuturesOrderType::Unknown => OrderType::Market,
    }
}
...
fn parse_time_in_force(tif: AsterTimeInForce) -> TimeInForce {
    match tif {
        ...
        AsterTimeInForce::Unknown => TimeInForce::Gtc,
    }
}
...
mod tests {
    ...
    use crate::{
        common::{
            ...
            enums::{AsterPriceMatch, AsterSelfTradePreventionMode},
            testing::load_fixture_string,
        },
        futures::websocket::streams::messages::{
            AsterFuturesAccountUpdateMsg, AsterFuturesAlgoUpdateMsg,
            AsterFuturesOrderUpdateMsg,
        },
    };
    ...
    fn instrument_id() -> InstrumentId {
        InstrumentId::from("ETHUSDT-PERP.ASTER")
    }

    fn account_id() -> AccountId {
        AccountId::from("ASTER-FUTURES-001")
    }
    ...
    fn test_parse_order_update_to_order_status_new() {
        let msg: AsterFuturesOrderUpdateMsg = load_user_data_fixture("order_update_new.json");
    ...
    fn test_parse_order_update_to_fill_report() {
        ...
        assert_eq!(
            msg.order.stp_mode,
            Some(AsterSelfTradePreventionMode::ExpireTaker),
        );
    ...
    fn test_parse_account_update() {
        let msg: AsterFuturesAccountUpdateMsg = load_user_data_fixture("account_update.json");
        let ts_init = UnixNanos::from(1_000_000_000u64);
    ...
    fn test_parse_algo_update_to_order_status_canceled() {
        let msg: AsterFuturesAlgoUpdateMsg = load_user_data_fixture("algo_update_canceled.json");
        let ts_init = UnixNanos::from(1_000_000_000u64);

        assert_eq!(
            msg.algo_order.stp_mode,
            Some(AsterSelfTradePreventionMode::ExpireMaker),
        );
        assert_eq!(msg.algo_order.price_match, Some(AsterPriceMatch::None));
    ...
    fn test_parse_algo_update_to_order_status_new_returns_none() {
        let msg: AsterFuturesAlgoUpdateMsg = load_user_data_fixture("algo_update_new.json");
        let report = parse_futures_algo_update_to_order_status(
            &msg.algo_order,
            msg.event_time,
            instrument_id(),
    ...
    fn test_decode_order_client_id() {
        let mut msg: AsterFuturesOrderUpdateMsg = load_user_data_fixture("order_update_new.json");
        let original = ClientOrderId::from("O-20200101-000000-000-000-1");
        msg.order.client_order_id = encode_broker_id(&original, ASTER_NAUTILUS_FUTURES_BROKER_ID);
    ...
    fn test_decode_algo_client_id() {
        let mut msg: AsterFuturesAlgoUpdateMsg =
        ...
        msg.algo_order.client_algo_id =
            encode_broker_id(&original, ASTER_NAUTILUS_FUTURES_BROKER_ID);
    ...
    fn test_parse_adl_fill_with_new_adl_status() {
        let msg: AsterFuturesOrderUpdateMsg = load_user_data_fixture("order_update_adl.json");
        let ts_init = UnixNanos::from(1_000_000_000u64);
    ...
    fn test_parse_order_status_new_adl_maps_to_filled() {
        let result = parse_order_status(AsterOrderStatus::NewAdl, false);
        assert_eq!(result, OrderStatus::Filled);
    }

    #[rstest]
    fn test_parse_order_status_new_insurance_maps_to_filled() {
        let result = parse_order_status(AsterOrderStatus::NewInsurance, false);
        assert_eq!(result, OrderStatus::Filled);
    }
    ...
    fn test_parse_insurance_fill_with_new_insurance_status() {
        ...
        assert!(msg.order.is_liquidation());
        assert!(msg.order.is_exchange_generated());
        assert_eq!(msg.order.order_status, AsterOrderStatus::NewInsurance);
    ...
    fn test_resolve_commission(
        ...
        let mut msg: AsterFuturesOrderUpdateMsg =
            load_user_data_fixture("order_update_calculated.json");
    ...
    #[case::with_venue_position_id(
        ...
        Some(PositionId::new("ETHUSDT-PERP.ASTER-LONG")),
    )]
