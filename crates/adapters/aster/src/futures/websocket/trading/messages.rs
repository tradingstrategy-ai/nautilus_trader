...
//! Aster Futures WebSocket Trading API message types.
//!
//! This module defines:
//! - [`AsterFuturesWsTradingCommand`]: Commands sent from the client to the handler.
//! - [`AsterFuturesWsTradingMessage`]: Output messages emitted by the handler to the client.
//! - Request/response structures for the Aster Futures WebSocket Trading API.

use nautilus_network::websocket::WebSocketClient;
use serde::{Deserialize, Serialize};

use crate::futures::http::{
    models::AsterFuturesOrder,
    query::{AsterCancelOrderParams, AsterModifyOrderParams, AsterNewOrderParams},
};
...
pub enum AsterFuturesWsTradingCommand {
...
#[derive(Debug, Clone)]
pub enum AsterFuturesWsTradingMessage {
    ...
    OrderAccepted {
        ...
        response: Box<AsterFuturesOrder>,
    },
...
pub enum AsterFuturesWsTradingRequestMeta {
...
pub struct AsterFuturesWsTradingRequest {
...
impl AsterFuturesWsTradingRequest {
...
///
/// Aster Futures WS API returns responses in this format.
#[derive(Debug, Clone, Deserialize)]
pub struct AsterFuturesWsTradingResponse {
    /// Request ID for correlation.
    pub id: String,
    ...
    pub error: Option<AsterFuturesWsTradingResponseError>,
}

/// Error details within a WebSocket Trading API response.
#[derive(Debug, Clone, Deserialize)]
pub struct AsterFuturesWsTradingResponseError {
    /// Error code from venue.
    pub code: i32,
    /// Error message from venue.
    pub msg: String,
...
/// WebSocket Trading API method names for Aster Futures.
pub mod method {
    /// Places a new order.
