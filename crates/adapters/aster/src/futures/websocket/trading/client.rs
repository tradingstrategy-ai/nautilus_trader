...
//! Aster Futures WebSocket Trading API client.
//!
//! ## Connection details
//!
//! - Endpoint: `ws-fapi.asterdex.com/ws-fapi/v1` (USD-M only)
...
use super::{
    ...
    handler::AsterFuturesWsTradingHandler,
    messages::{AsterFuturesWsTradingCommand, AsterFuturesWsTradingMessage},
};
use crate::{
    common::{
        consts::{ASTER_API_KEY_HEADER, ASTER_FUTURES_USD_WS_API_URL},
...
///
/// Aster Futures WebSocket API: 1200 requests per minute per IP (20/sec).
pub static ASTER_FUTURES_WS_RATE_LIMIT_KEY_ORDER: LazyLock<[Ustr; 1]> =
    LazyLock::new(|| [Ustr::from("futures_order")]);

/// Returns the Aster Futures WebSocket API order rate limit quota (1200 per minute).
// Constant values are provably valid
#[allow(clippy::missing_panics_doc)]
#[must_use]
pub fn aster_futures_ws_order_quota() -> Quota {
...
#[derive(Clone)]
pub struct AsterFuturesWsTradingClient {
    url: String,
    ...
    cmd_tx: Arc<
        tokio::sync::RwLock<tokio::sync::mpsc::UnboundedSender<AsterFuturesWsTradingCommand>>,
    >,
    out_rx:
        Arc<Mutex<Option<tokio::sync::mpsc::UnboundedReceiver<AsterFuturesWsTradingMessage>>>>,
    task_handle: Option<Arc<tokio::task::JoinHandle<()>>>,
    request_id_counter: Arc<AtomicU64>,
    cancellation_token: CancellationToken,
}

impl Debug for AsterFuturesWsTradingClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(AsterFuturesWsTradingClient))
            .field("url", &self.url)
...
impl AsterFuturesWsTradingClient {
    /// Creates a new [`AsterFuturesWsTradingClient`] instance.
    #[must_use]
    pub fn new(
        url: Option<String>,
        api_key: String,
        ...
        let url = url.unwrap_or_else(|| ASTER_FUTURES_USD_WS_API_URL.to_string());
    ...
    #[allow(clippy::missing_panics_doc)]
    pub async fn connect(&mut self) -> AsterFuturesWsApiResult<()> {
        ...
        let keyed_quotas = vec![(
            ASTER_FUTURES_WS_RATE_LIMIT_KEY_ORDER[0]
                .as_str()
                .to_string(),
            aster_futures_ws_order_quota(),
        )];
        ...
        let client = WebSocketClient::connect(
            ...
            None,
            keyed_quotas,
            Some(aster_futures_ws_order_quota()),
        )
        .await
        .map_err(|e| AsterFuturesWsApiError::ConnectionError(e.to_string()))?;
        ...
        let mut handler =
            AsterFuturesWsTradingHandler::new(signal, cmd_rx, raw_rx, out_tx, credential);

        self.cmd_tx
            .read()
            .await
            .send(AsterFuturesWsTradingCommand::SetClient(client))
            .map_err(|e| AsterFuturesWsApiError::HandlerUnavailable(e.to_string()))?;
    ...
    pub async fn disconnect(&mut self) {
        ...
        if let Err(e) = self
            ...
            .read()
            .await
            .send(AsterFuturesWsTradingCommand::Disconnect)
        {
    ...
    ) -> AsterFuturesWsApiResult<String> {
    ...
    pub async fn place_order_with_id(
        ...
        let cmd = AsterFuturesWsTradingCommand::PlaceOrder { id, params };
    ...
    pub async fn cancel_order_with_id(
        ...
        let cmd = AsterFuturesWsTradingCommand::CancelOrder { id, params };
        self.send_cmd(cmd).await
    }
    ...
    pub async fn modify_order_with_id(
        ...
        let cmd = AsterFuturesWsTradingCommand::ModifyOrder { id, params };
        self.send_cmd(cmd).await
    }
    ...
    pub async fn cancel_all_orders(
        ...
        let id = self.next_request_id();
        let cmd = AsterFuturesWsTradingCommand::CancelAllOrders {
            id: id.clone(),
            symbol: symbol.into(),
        };
        self.send_cmd(cmd).await?;
    ...
    pub async fn recv(&self) -> Option<AsterFuturesWsTradingMessage> {
        let rx_opt = {
            let mut rx_guard = self.out_rx.lock().expect("Mutex poisoned");
            rx_guard.take()
        };
    ...
    async fn send_cmd(&self, cmd: AsterFuturesWsTradingCommand) -> AsterFuturesWsApiResult<()> {
        self.cmd_tx
            .read()
            .await
            .send(cmd)
