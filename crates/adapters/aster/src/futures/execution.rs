...
//! Live execution client implementation for the Aster Futures adapter.
...
use super::{
    http::{
        AsterFuturesHttpError,
        client::{AsterFuturesHttpClient, AsterFuturesInstrument, is_algo_order_type},
        models::{BatchOrderResult, AsterPositionRisk},
        query::{
            BatchCancelItem, AsterAllOrdersParamsBuilder, AsterOpenOrdersParamsBuilder,
            AsterOrderQueryParamsBuilder, AsterPositionRiskParamsBuilder,
            AsterSetLeverageParams, AsterSetMarginTypeParams, AsterUserTradesParamsBuilder,
        },
    },
    websocket::{
        streams::{
            client::AsterFuturesWebSocketClient,
            messages::{
                AsterExecutionType, AsterFuturesAlgoUpdateMsg, AsterFuturesOrderUpdateMsg,
                AsterFuturesWsStreamsMessage,
            },
            parse_exec::{
        ...
        trading::{
            client::AsterFuturesWsTradingClient, messages::AsterFuturesWsTradingMessage,
        },
    },
};
use crate::{
    common::{
        consts::{
            ASTER_FUTURES_USD_WS_API_TESTNET_URL, ASTER_FUTURES_USD_WS_API_URL,
            ASTER_GTX_ORDER_REJECT_CODE, ASTER_NAUTILUS_FUTURES_BROKER_ID, ASTER_VENUE,
        },
        ...
        enums::{
            AsterEnvironment, AsterPositionSide, AsterPriceMatch, AsterProductType,
            AsterSide, AsterTimeInForce, AsterWorkingType,
        },
        symbol::format_aster_symbol,
    },
    config::AsterExecClientConfig,
    futures::http::{
        client::order_type_to_aster_futures,
        models::AsterFuturesAccountInfo,
        query::{
            AsterCancelOrderParamsBuilder, AsterModifyOrderParamsBuilder, AsterNewOrderParams,
        },
    },
};
...
/// Live execution client for Aster Futures trading.
///
/// Implements the [`ExecutionClient`] trait for order management on Aster
...
#[derive(Debug)]
pub struct AsterFuturesExecutionClient {
    ...
    http_client: AsterFuturesHttpClient,
    ws_client: Option<AsterFuturesWebSocketClient>,
    ws_trading_client: Option<AsterFuturesWsTradingClient>,
...
impl AsterFuturesExecutionClient {
    /// Creates a new [`AsterFuturesExecutionClient`].
    ///
    /// # Errors
    ///
    /// Returns an error if the HTTP client fails to initialize or credentials are missing.
    pub fn new(core: ExecutionClientCore, config: AsterExecClientConfig) -> anyhow::Result<Self> {
        let product_type = config
            .product_types
            .iter()
            .find(|pt| matches!(pt, AsterProductType::UsdM | AsterProductType::CoinM))
            .copied()
        ...
        let ws_trading_client = if config.use_ws_trading && product_type == AsterProductType::UsdM
        {
            let ws_trading_url =
                config
                    .base_url_ws_trading
                    ...
                    .or_else(|| match config.environment {
                        AsterEnvironment::Testnet => {
                            Some(ASTER_FUTURES_USD_WS_API_TESTNET_URL.to_string())
                        }
                        _ => Some(ASTER_FUTURES_USD_WS_API_URL.to_string()),
                    });

            Some(AsterFuturesWsTradingClient::new(
                ws_trading_url,
                api_key.clone(),
                api_secret.clone(),
                None, // heartbeat
        ...
        .context("failed to construct Aster Futures WebSocket client")?;
    ...
    #[doc(hidden)]
    #[must_use]
    pub fn instruments_cache(&self) -> Arc<DashMap<ustr::Ustr, AsterFuturesInstrument>> {
    ...
    fn determine_position_side(
        ...
        Some(if reduce_only {
            ...
            match order_side {
                OrderSide::Buy => AsterPositionSide::Short,
                OrderSide::Sell => AsterPositionSide::Long,
                _ => AsterPositionSide::Both,
            }
        } else {
            // Opening: Buy opens Long, Sell opens Short
            match order_side {
                OrderSide::Buy => AsterPositionSide::Long,
                OrderSide::Sell => AsterPositionSide::Short,
                _ => AsterPositionSide::Both,
            }
        })
    }

    /// Converts Aster futures account info to Nautilus account state.
    fn create_account_state(&self, account_info: &AsterFuturesAccountInfo) -> AccountState {
    ...
    fn create_account_state_from(
            ...
            && (!initial_dec.is_zero() || !maint_dec.is_zero())
        {
            ...
            let margin_instrument_id = InstrumentId::new(Symbol::new("ACCOUNT"), *ASTER_VENUE);
    ...
    async fn refresh_account_state(&self) -> anyhow::Result<AccountState> {
        let account_info = match self.http_client.query_account().await {
            Ok(info) => info,
            Err(e) => {
                log::error!("Aster Futures account state request failed: {e}");
                anyhow::bail!("Aster Futures account state request failed: {e}");
            }
        };
    ...
    fn update_account_state(&self) {
        ...
        self.spawn_task("query_account", async move {
            let account_info = http_client
                .query_account()
                .await
                .context("Aster Futures account state request failed")?;
            let account_state =
    ...
    fn submit_order_internal(&self, cmd: &SubmitOrder) -> anyhow::Result<()> {
        ...
        let price_match = cmd
            ...
            .map(AsterPriceMatch::from_param)
            .transpose()?;
        ...
        let working_type = match trigger_type {
            Some(TriggerType::MarkPrice) => Some(AsterWorkingType::MarkPrice),
            Some(TriggerType::LastPrice | TriggerType::Default) => {
                Some(AsterWorkingType::ContractPrice)
            }
            _ => None,
        };
        ...
        if self.ws_trading_active() && !use_algo_api {
            ...
            let client_id_str =
                encode_broker_id(&client_order_id, ASTER_NAUTILUS_FUTURES_BROKER_ID);
        ...
        self.spawn_task("submit_order", async move {
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    // reached Aster, WebSocket updates will still arrive. The order
                    // will be cleaned up via WebSocket rejection or reconciliation.
                    let due_post_only =
                        e.downcast_ref::<AsterFuturesHttpError>()
                            .is_some_and(|be| {
                                matches!(
                                    be,
                                    AsterFuturesHttpError::AsterError { code, .. }
                                        if *code == ASTER_GTX_ORDER_REJECT_CODE
                                )
                            });
                    let ts_now = clock.get_time_ns();
    ...
    fn cancel_order_internal(&self, cmd: &CancelOrder) {
        ...
        if self.ws_trading_active() && !use_algo_cancel {
            ...
            let mut cancel_builder = AsterCancelOrderParamsBuilder::default();
    ...
    /// Creates a position status report from Aster position risk data.
    fn create_position_report(
        &self,
        position: &AsterPositionRisk,
    ...
    async fn apply_futures_config(&self) -> anyhow::Result<()> {
        if let Some(ref leverages) = self.config.futures_leverages {
            for (symbol, leverage) in leverages {
                let params = AsterSetLeverageParams {
                    symbol: symbol.clone(),
                    leverage: *leverage,
                    recv_window: None,
                };
        ...
        if let Some(ref margin_types) = self.config.futures_margin_types {
            for (symbol, margin_type) in margin_types {
                let params = AsterSetMarginTypeParams {
                    symbol: symbol.clone(),
...
#[async_trait(?Send)]
impl ExecutionClient for AsterFuturesExecutionClient {
    ...
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        let _instruments = if self.core.instruments_initialized() {
            ...
            let instruments = self
                ...
                .request_instruments()
                .await
                .context("failed to request Aster Futures instruments")?;

            if instruments.is_empty() {
                log::warn!("No instruments returned for Aster Futures");
            } else {
        ...
        if let Some(ref mut ws_client) = self.ws_client {
            log::info!("Connecting to Aster Futures user data stream WebSocket...");
            ws_client.connect().await.map_err(|e| {
                log::error!("Aster Futures WebSocket connection failed: {e:?}");
                anyhow::anyhow!("failed to connect Aster Futures WebSocket: {e}")
            })?;
            log::info!("Aster Futures WebSocket connected");
        ...
        let account_state = self
            ...
            .await
            .context("failed to request Aster Futures account state")?;
        ...
        if let Some(ref mut ws_trading) = self.ws_trading_client {
            match ws_trading.connect().await {
                Ok(()) => {
                    log::info!("Connected to Aster Futures WS trading API");
    ...
    async fn generate_order_status_report(
        ...
        let mut builder = AsterOrderQueryParamsBuilder::default();
        ...
        match self.http_client.query_order(&params).await {
            ...
            Err(AsterFuturesHttpError::AsterError { code: -2013, .. }) => {
    ...
    async fn generate_order_status_reports(
        ...
        if cmd.open_only {
            ...
            let mut builder = AsterOpenOrdersParamsBuilder::default();
            ...
            let mut builder = AsterAllOrdersParamsBuilder::default();
            builder.symbol(symbol);
    ...
    async fn generate_fill_reports(
        ...
        let Some(instrument_id) = cmd.instrument_id else {
            log::warn!("generate_fill_reports requires instrument_id for Aster Futures");
            return Ok(Vec::new());
        };
        ...
        let mut builder = AsterUserTradesParamsBuilder::default();
        builder.symbol(symbol);
    ...
    async fn generate_position_status_reports(
        ...
        let mut builder = AsterPositionRiskParamsBuilder::default();
    ...
    fn start(&mut self) -> anyhow::Result<()> {
        ...
        get_runtime().spawn(async move {
            match http_client.request_instruments().await {
                ...
                Err(e) => {
                    log::error!("Failed to request Aster Futures instruments: {e}");
                }
            }
        });
    ...
    fn submit_order(&self, cmd: &SubmitOrder) -> anyhow::Result<()> {
        ...
        if let Some(offset_type) = order.trailing_offset_type() {
            if offset_type != TrailingOffsetType::BasisPoints {
                anyhow::bail!(
                    "Aster only supports TrailingOffsetType::BasisPoints, received {offset_type:?}"
                );
            }
        ...
        if close_position {
            ...
            if !matches!(
                ...
                anyhow::bail!(
                    "`close_position` is not supported for order type {order_type:?} on Aster"
                );
            }

            if order.is_reduce_only() {
                anyhow::bail!("`close_position` cannot be combined with `reduce_only` on Aster");
            }
        }

        if let Some(pm_str) = cmd.params.as_ref().and_then(|p| p.get_str("price_match")) {
            AsterPriceMatch::from_param(pm_str)?;
            let order_type = order.order_type();
            anyhow::ensure!(
                !order.is_post_only(),
    ...
    fn submit_order_list(&self, cmd: &SubmitOrderList) -> anyhow::Result<()> {
        log::warn!(
            "submit_order_list not yet implemented for Aster Futures (got {} orders)",
    ...
    fn modify_order(&self, cmd: &ModifyOrder) -> anyhow::Result<()> {
        ...
        if self.ws_trading_active() {
            ...
            let mut modify_builder = AsterModifyOrderParamsBuilder::default();
            modify_builder
                .symbol(format_aster_symbol(&instrument_id))
                .side(aster_side)
...
fn dispatch_ws_message(
    msg: AsterFuturesWsStreamsMessage,
    emitter: &ExecutionEventEmitter,
    http_client: &AsterFuturesHttpClient,
    account_id: AccountId,
    product_type: AsterProductType,
    ...
    match msg {
        AsterFuturesWsStreamsMessage::OrderUpdate(update) => {
        ...
        AsterFuturesWsStreamsMessage::AlgoUpdate(update) => {
        ...
        AsterFuturesWsStreamsMessage::AccountConfigUpdate(cfg) => {
        ...
        AsterFuturesWsStreamsMessage::ListenKeyExpired => {
            log::warn!("Listen key expired, awaiting reconnection");
        }
        AsterFuturesWsStreamsMessage::Reconnected => {
            log::info!("User data stream WebSocket reconnected");
        }
        AsterFuturesWsStreamsMessage::Error(err) => {
            log::error!(
        ...
        AsterFuturesWsStreamsMessage::AggTrade(_)
        | AsterFuturesWsStreamsMessage::Trade(_)
        | AsterFuturesWsStreamsMessage::BookTicker(_)
        | AsterFuturesWsStreamsMessage::DepthUpdate(_)
        | AsterFuturesWsStreamsMessage::MarkPrice(_)
        | AsterFuturesWsStreamsMessage::Kline(_)
        | AsterFuturesWsStreamsMessage::ForceOrder(_)
        | AsterFuturesWsStreamsMessage::Ticker(_) => {}
    }
}
...
fn dispatch_order_update(
    msg: &AsterFuturesOrderUpdateMsg,
    emitter: &ExecutionEventEmitter,
    http_client: &AsterFuturesHttpClient,
    account_id: AccountId,
    product_type: AsterProductType,
    ...
    if order.is_exchange_generated() {
        let is_linear = cached_instrument
            .as_ref()
            .map_or(product_type == AsterProductType::UsdM, |inst| {
                matches!(inst.value(), AsterFuturesInstrument::UsdM(_))
            });
...
///
/// Derives a venue position ID from the instrument and Aster position side.
///
/// Returns `None` when `use_position_ids` is false.
fn make_venue_position_id(
    use_position_ids: bool,
    instrument_id: InstrumentId,
    position_side: AsterPositionSide,
) -> Option<PositionId> {
    if !use_position_ids {
        return None;
    }
    ...
    let side = match position_side {
        AsterPositionSide::Long => "LONG",
        AsterPositionSide::Short => "SHORT",
        AsterPositionSide::Both => "BOTH",
        _ => "UNKNOWN",
    };
...
fn dispatch_algo_update(
    ...
    use crate::common::enums::AsterAlgoStatus;
    ...
    match algo_data.algo_status {
        ...
        AsterAlgoStatus::Canceled | AsterAlgoStatus::Expired => {
...
fn dispatch_ws_trading_message(
    msg: AsterFuturesWsTradingMessage,
    emitter: &ExecutionEventEmitter,
    account_id: AccountId,
    clock: &'static AtomicTime,
    dispatch_state: &WsDispatchState,
    ...
    match msg {
        AsterFuturesWsTradingMessage::OrderAccepted {
            request_id,
            response,
        } => {
            dispatch_state.pending_requests.remove(&request_id);
        ...
        AsterFuturesWsTradingMessage::OrderRejected {
            request_id,
            code,
            msg,
        } => {
            ...
            if let Some((_, pending)) = dispatch_state.pending_requests.remove(&request_id) {
                ...
                if let Some(identity) = identity {
                    let due_post_only = i64::from(code) == ASTER_GTX_ORDER_REJECT_CODE;
                    let ts_now = clock.get_time_ns();
                    let rejected = OrderRejected::new(
                        emitter.trader_id(),
                        identity.strategy_id,
        ...
        AsterFuturesWsTradingMessage::OrderCanceled {
            request_id,
            response,
        } => {
            dispatch_state.pending_requests.remove(&request_id);
        ...
        AsterFuturesWsTradingMessage::CancelRejected {
            request_id,
            code,
            msg,
        } => {
        ...
        AsterFuturesWsTradingMessage::OrderModified {
            request_id,
            response,
        } => {
            dispatch_state.pending_requests.remove(&request_id);
        ...
        AsterFuturesWsTradingMessage::ModifyRejected {
            request_id,
            code,
            msg,
        } => {
        ...
        AsterFuturesWsTradingMessage::AllOrdersCanceled { request_id } => {
            dispatch_state.pending_requests.remove(&request_id);
            log::debug!("WS all orders canceled: request_id={request_id}");
        }
        AsterFuturesWsTradingMessage::Connected => {
            log::info!("WS trading API connected");
        }
        AsterFuturesWsTradingMessage::Reconnected => {
            log::info!("WS trading API reconnected");
        }
        AsterFuturesWsTradingMessage::Error(err) => {
...
fn trailing_offset_to_callback_rate(offset: Decimal) -> anyhow::Result<Decimal> {
    ...
    if rate < min_rate || rate > max_rate {
        anyhow::bail!("callbackRate {rate}% out of Aster range [{min_rate}, {max_rate}]");
    }
...
mod tests {
    ...
    #[rstest]
    #[case::long(AsterPositionSide::Long, "ETHUSDT-PERP.ASTER-LONG")]
    #[case::short(AsterPositionSide::Short, "ETHUSDT-PERP.ASTER-SHORT")]
    #[case::both(AsterPositionSide::Both, "ETHUSDT-PERP.ASTER-BOTH")]
    #[case::unknown(AsterPositionSide::Unknown, "ETHUSDT-PERP.ASTER-UNKNOWN")]
    fn test_make_venue_position_id_enabled(
        #[case] side: AsterPositionSide,
        #[case] expected: &str,
    ) {
        let instrument_id = InstrumentId::from("ETHUSDT-PERP.ASTER");
        let result = make_venue_position_id(true, instrument_id, side);
        assert_eq!(result, Some(PositionId::from(expected)));
    }

    #[rstest]
    #[case::long(AsterPositionSide::Long)]
    #[case::short(AsterPositionSide::Short)]
    #[case::both(AsterPositionSide::Both)]
    fn test_make_venue_position_id_disabled(#[case] side: AsterPositionSide) {
        let instrument_id = InstrumentId::from("ETHUSDT-PERP.ASTER");
    ...
    fn test_trailing_offset_to_callback_rate_rejects_out_of_range_values() {
        ...
        assert_eq!(
            error.to_string(),
            "callbackRate 0.05% out of Aster range [0.1, 10.0]"
        );
    }
    ...
    fn test_dispatch_order_update_skips_duplicate_tracked_trade() {
        let clock = get_atomic_clock_realtime();
        let msg: AsterFuturesOrderUpdateMsg = load_user_data_fixture("order_update_trade.json");
    ...
    fn test_dispatch_order_update_skips_duplicate_exchange_generated_fill() {
        let clock = get_atomic_clock_realtime();
        let msg: AsterFuturesOrderUpdateMsg =
    ...
    fn test_dispatch_ws_trading_message_emits_cancel_rejected_and_clears_pending_request() {
            ...
            .expect("Cancel rejection event should be emitted")
        {
            ExecutionEvent::Order(OrderEventAny::CancelRejected(event)) => {
                ...
                assert_eq!(event.account_id, Some(AccountId::from("ASTER-001")));
    ...
    fn create_test_http_client(clock: &'static AtomicTime) -> AsterFuturesHttpClient {
        AsterFuturesHttpClient::new(
            AsterProductType::UsdM,
