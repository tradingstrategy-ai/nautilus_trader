...
use crate::{
    common::enums::{AsterEnvironment, AsterMarginType, AsterProductType},
...
mod tests {
    ...
    fn test_data_client_py_new_uses_explicit_overrides() {
        let config = AsterDataClientConfig::py_new(
            Some(vec![AsterProductType::UsdM]),
            Some(AsterEnvironment::Testnet),
    ...
    fn test_exec_client_py_new_preserves_explicit_overrides() {
        ...
        use crate::common::enums::AsterMarginType;

        let leverages = HashMap::from([("BTCUSDT".to_string(), 20)]);
        let margin_types = HashMap::from([("BTCUSDT".to_string(), AsterMarginType::Cross)]);

        let config = AsterExecClientConfig::py_new(
            TraderId::from("TRADER-002"),
            AccountId::from("ASTER-002"),
            Some(vec![AsterProductType::UsdM]),
            Some(AsterEnvironment::Demo),
            Some("https://http.example".to_string()),
