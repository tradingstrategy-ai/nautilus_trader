...
//! Deterministic two-way encoder for Aster Link broker ID prefixing.
//!
//! The Aster broker ID is automatically prefixed to all system-generated
//! client order IDs for every order placed through the Aster adapter. This
...
//!
//! Aster's [Link and Trade] program requires the `newClientOrderId`
//! parameter to start with `x-{BROKER_ID}` for order attribution. Aster
...
//!
//! [Link and Trade]: https://developers.asterdex.com/docs/aster_link/link-and-trade
//!
//! # Wire format
//!
//! ```text
...
//! Use `cargo test --release -p nautilus-aster --lib -- bench_encode_decode_timing --nocapture`
...
/// Maximum `newClientOrderId` length allowed by the Aster API.
...
/// Encodes a `ClientOrderId` into a Aster-compatible string with broker ID
/// prefix.
///
...
mod tests {
    ...
    use super::{super::consts::ASTER_NAUTILUS_SPOT_BROKER_ID, *};

    const TEST_BROKER_ID: &str = ASTER_NAUTILUS_SPOT_BROKER_ID;
    ...
    #[rstest]
    fn test_encoded_chars_are_aster_valid() {
        let valid = |c: char| {
        ...
        for id_str in ids {
            ...
            assert!(
                ...
                "'{encoded}' contains invalid Aster characters"
            );
        }
    }
    ...
    fn test_end_to_end_submit_and_receive(#[case] original_id: &str) {
        ...
        // Simulate submit: encode the client order ID for Aster
        let encoded = encode_broker_id(&client_order_id, broker_id);
        assert!(encoded.len() <= 36, "encoded len {} > 36", encoded.len());

        // Simulate receive: Aster echoes the encoded ID back in a response
