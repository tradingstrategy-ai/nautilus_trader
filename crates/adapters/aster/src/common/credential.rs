...
//! Aster API credential handling and request signing.
//!
...
//! (`ASTER_API_KEY`/`ASTER_API_SECRET`). The deprecated `*_ED25519_*`
...
use super::enums::{AsterEnvironment, AsterProductType};

/// Resolves API credentials from config or environment variables.
///
/// Checks standard environment variables:
/// - Live: `ASTER_API_KEY` / `ASTER_API_SECRET`
/// - Testnet (Spot): `ASTER_TESTNET_API_KEY` / `ASTER_TESTNET_API_SECRET`
/// - Testnet (Futures): `ASTER_FUTURES_TESTNET_API_KEY` / `ASTER_FUTURES_TESTNET_API_SECRET`
/// - Demo: `ASTER_DEMO_API_KEY` / `ASTER_DEMO_API_SECRET`
///
...
pub fn resolve_credentials(
    ...
    let (deprecated_key_var, deprecated_secret_var, standard_key_var, standard_secret_var) =
        match environment {
            AsterEnvironment::Testnet => match product_type {
                AsterProductType::Spot
                | AsterProductType::Margin
                | AsterProductType::Options => (
                    "ASTER_TESTNET_ED25519_API_KEY",
                    "ASTER_TESTNET_ED25519_API_SECRET",
                    "ASTER_TESTNET_API_KEY",
                    "ASTER_TESTNET_API_SECRET",
                ),
                AsterProductType::UsdM | AsterProductType::CoinM => (
                    "ASTER_FUTURES_TESTNET_ED25519_API_KEY",
                    "ASTER_FUTURES_TESTNET_ED25519_API_SECRET",
                    "ASTER_FUTURES_TESTNET_API_KEY",
                    "ASTER_FUTURES_TESTNET_API_SECRET",
                ),
            },

            // Demo shares API keys across all product types
            AsterEnvironment::Demo => ("", "", "ASTER_DEMO_API_KEY", "ASTER_DEMO_API_SECRET"),
            AsterEnvironment::Mainnet => (
                "ASTER_ED25519_API_KEY",
                "ASTER_ED25519_API_SECRET",
                "ASTER_API_KEY",
                "ASTER_API_SECRET",
            ),
        };
...
/// Aster API credentials for signing requests (HMAC SHA256).
///
/// Uses HMAC SHA256 with hexadecimal encoding, as required by Aster REST API signing.
...
/// Aster Ed25519 credentials for WebSocket API authentication.
///
...
///
/// Aster supports two signing methods:
...
mod tests {
    ...
    // Official Aster test vectors from:
    // https://github.com/aster/aster-signature-examples
    const ASTER_TEST_SECRET: &str =
        "NhqPtmdSJYdKjVHjA7PZj4Mge3R5YNiP1e3UZjInClVN65XAbvqqM6A7H5fATj0j";

    #[rstest]
    fn test_sign_matches_aster_test_vector_simple() {
        let cred = Credential::new("test_key".to_string(), ASTER_TEST_SECRET.to_string());
    ...
    #[rstest]
    fn test_sign_matches_aster_test_vector_order() {
        let cred = Credential::new("test_key".to_string(), ASTER_TEST_SECRET.to_string());
