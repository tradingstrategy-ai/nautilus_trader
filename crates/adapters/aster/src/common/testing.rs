...
//! Test fixtures and utilities for Aster adapter tests.
...
pub fn load_fixture_string(relpath: &str) -> String {
    ...
    fs::read_to_string(&path)
        .unwrap_or_else(|_| panic!("Failed to read Aster test fixture: {}", path.display()))
}
...
pub fn load_json_fixture(relpath: &str) -> Value {
    ...
    serde_json::from_str(&content).expect("Invalid JSON in Aster test fixture")
}
...
///
/// Aster docs examples for user-data streams wrap the payload in an
