...
/// Represents a Aster bar (kline/candlestick) with additional Aster-specific fields.
///
...
impl AsterBar {
    /// Creates a new [`AsterBar`] instance.
...
impl HasTsInit for AsterBar {
    fn ts_init(&self) -> UnixNanos {
        self.ts_init
    }
}

impl CustomDataTrait for AsterBar {
...
mod tests {
    ...
    fn stub_aster_bar() -> AsterBar {
        AsterBar::new(
            BarType::from("BTCUSDT.ASTER-1-MINUTE-LAST-EXTERNAL"),
    ...
    #[rstest]
    fn test_type_name() {
        let bar = stub_aster_bar();
        assert_eq!(bar.type_name(), "AsterBar");
        assert_eq!(AsterBar::type_name_static(), "AsterBar");
    }
    ...
    fn test_json_round_trip() {
        ...
        let restored = AsterBar::from_json(value).unwrap();
        let restored_bar = restored.as_any().downcast_ref::<AsterBar>().unwrap();
