...
use nautilus_model::{
    ...
    enums::{
        BarAggregation, LiquiditySide, OrderSide, OrderStatus, OrderType, TimeInForce, TriggerType,
    },
    identifiers::{
...
///
/// Aster SBE encodes values as `mantissa * 10^exponent` where `exponent` is a global
...
fn parse_sbe_price_filter(filter: &AsterPriceFilterSbe) -> (Price, Option<Price>, Option<Price>) {
...
/// Maps Aster SBE order status to Nautilus order status.
#[must_use]
...
/// Maps Aster SBE order type to Nautilus order type.
#[must_use]
...
/// Maps Aster SBE order side to Nautilus order side.
#[must_use]
...
/// Maps Aster SBE time in force to Nautilus time in force.
#[must_use]
...
/// Parses a Aster SBE order response into a Nautilus `OrderStatusReport`.
///
/// # Errors
///
...
/// Parses a Aster new order response (SBE) into a Nautilus `OrderStatusReport`.
///
/// # Errors
///
/// Returns an error if any field cannot be parsed.
pub fn parse_new_order_response_sbe(
    response: &AsterNewOrderResponse,
    account_id: AccountId,
    instrument: &InstrumentAny,
    broker_id: &str,
    ts_init: UnixNanos,
...
/// Parses a Aster SBE account trade into a Nautilus `FillReport`.
///
/// # Errors
///
/// Returns an error if any field cannot be parsed.
pub fn parse_fill_report_sbe(
    trade: &AsterAccountTrade,
    account_id: AccountId,
    instrument: &InstrumentAny,
...
/// Parses Aster klines (candlesticks) into Nautilus Bar objects.
///
/// # Errors
///
...
mod tests {
    ...
    mod sbe_precision_tests {
        ...
        fn test_parse_sbe_price_filter_ethusdc() {
            let filter = AsterPriceFilterSbe {
                price_exponent: -8,
                min_price: 1_000_000,
        ...
        fn test_parse_sbe_lot_size_filter_ethusdc() {
            let filter = AsterLotSizeFilterSbe {
                qty_exponent: -8,
                min_qty: 10_000,
